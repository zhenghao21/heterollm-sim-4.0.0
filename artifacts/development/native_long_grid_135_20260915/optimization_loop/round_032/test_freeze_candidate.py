"""R32 synthetic closure checks. No actual freeze, model, native or prediction."""
from pathlib import Path
import copy, hashlib, importlib.util, json, subprocess, sys
from types import SimpleNamespace
import pytest
spec=importlib.util.spec_from_file_location("r32_freeze_tests",Path(__file__).with_name("freeze_candidate.py"))
...
def test_snapshot_uses_all_git_blobs_never_worktree_and_refuses_overwrite(tmp_path,monkeypatch):
    names=["src/heterollm_sim/planner.py","src/heterollm_sim/llama_graph_runtime.py"]
    monkeypatch.setattr(d,"git_members",lambda c:names)
    monkeypatch.setattr(d,"git_blobs",lambda c,n:{name:b"committed LF\n" for name in n})
    monkeypatch.setattr(d,"ROOT",tmp_path/"dirty")
    for name in names:write(d.ROOT/name,b"unreviewed WIP\r\n")
    refs=d.snapshot("a"*40,tmp_path/"snapshot")
    assert len(refs)==2 and all(r["origin"]=="reviewed_git_blob" and r["commit"]=="a"*40 for r in refs)
    assert all((tmp_path/"snapshot"/name).read_bytes()==b"committed LF\n" for name in names)
    with pytest.raises(FileExistsError):d.snapshot("a"*40,tmp_path/"snapshot")
...
def test_archive_accepts_omitted_or_explicit_legacy_but_rejects_nominal():
    spec=importlib.util.spec_from_file_location("r32_archive_test",d.P/"postprocess/archive_verified.py")
...
def origin_checker():
    spec=importlib.util.spec_from_file_location("r32_origin_tests",d.P/"verify_frozen_preflight.py")
    module=importlib.util.module_from_spec(spec);spec.loader.exec_module(module)
    return module
