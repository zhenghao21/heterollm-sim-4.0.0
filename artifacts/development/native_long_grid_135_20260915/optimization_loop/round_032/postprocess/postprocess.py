"""Presentation only, after existing R32 full262 barrier AND both scores.
...
def validate_scoring_receipt(base):
    # R32's frozen scorer receipt declares gate_B, not a blind field.
    require(base.get("schema") == "r32-two-arm-scoring-receipt/v1" and base.get("denominator_per_arm") == 131 and
...
def load_completed():
    ...
    r = load_module("r32_postprocess_runner", ROUND/"run_candidate.py")
    s = r.s
    ...
    require(grouped.get("schema") == "r32-grouped-paired-errors/v1" and grouped.get("fixed_cells") == 131 and
    ...
    g = load_module("r32_postprocess_grouped", ROUND/"summarize_groups.py")
...
def heatmaps(context, output):
    ...
    ape_cmap = LinearSegmentedColormap.from_list("r32_error", [(0,"#e8f4ef"),(.09999,"#6bb69a"),(.1,"#fce4a3"),(.2,"#f3b15e"),(.5,"#d76c5e"),(1,"#913446")])
    ape_cmap.set_bad("#e4e7eb")
    delta_cmap = LinearSegmentedColormap.from_list("r32_delta", ["#17614e", "#f7f7f7", "#a63240"]); delta_cmap.set_bad("#e4e7eb")
    ...
    for view, groups in panels.items():
        ...
        fig.suptitle("R32 GPU consumer frontend comparison | "+title,fontsize=15,y=.995)
    ...
    receipt={"schema":"r32-presentation-only-heatmaps/v1","created_utc":now(),"source_evidence":context["evidence"],"postprocessor_ref":ref(__file__),
