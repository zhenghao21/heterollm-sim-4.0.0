"""R32 clean-Git frontend ownership comparison; fixed native, no prediction at freeze."""
...
s = load("r32_saved_evidence", LOOP / "round_023/summarize_ablation.py")
r = load("r32_reviewed_recipe", LOOP / "round_023/evaluate_candidate.py")
...
def natural_process(argv, *, capture_output=False, text=False, stdout=None, stderr=None, check=False, purpose="helper", input=None):
    ...
    start_ref = write_new(journal / "start.json", {"schema": "r32-process-start/v1", "created_utc": now(),
    ...
    note("finish.json", {"schema": "r32-process-finish/v1", "start_ref": start_ref,
...
def snapshot(commit, destination):
    ...
    for name in names:
        ...
        refs.append({"relative": name, "origin": "reviewed_git_blob", "commit": commit, "ref": s.reference(target)})
    return refs
...
def environment_identity():
    ...
    return {"schema": "r32-python-environment/v1", "executable": s.reference(sys.executable),
...
def native_lock():
    verifier = load("r32_native_lock", ROOT / "tools/verify_fixed_native.py")
...
def read_protocol():
    ...
    s.require(protocol.get("schema") == "r32-clean-git-frontend-inputs/v1" and protocol.get("arms") == list(ARMS)
    ...
    for arm in ARMS:
        ...
        summaries[arm] = {}
        content = git_blobs(commits[arm], names)
        for row in rows:
            found = s.verify_reference(row["ref"])
            s.require(Path(found["path"]).resolve() == root / row["relative"], "source escaped exact execution root")
            s.require(row.get("origin") == "reviewed_git_blob" and row.get("commit") == commits[arm], "source origin differs")
...
def validate_preflight(stage, arm, reference, protocol_ref):
    ...
    s.require(receipt.get("schema") == "r32-frozen-source-preflight/v1" and receipt.get("stage") == stage
...
def freeze_arm(arm):
    ...
    api = load("r32_" + arm + "_snapshot_api", source / "tools/predict_stable_native_dataset.py")
    origins = load("r32_origin_check", P / "verify_frozen_preflight.py")
...
def freeze(commit):
    ...
    refs = {arm: snapshot(commits[arm], P / "execution_source" / arm) for arm in ARMS}
    ...
    protocol = {"schema": "r32-clean-git-frontend-inputs/v1", "created_utc": now(), "reviewed_commit": commit,
        ...
        "target_latency_fitting": False, "source_policy": "117 raw Git blobs per arm; exact planner-only reviewed diff; no worktree source",
        ...
        "prior_campaigns": "R30 and earlier preserved; not retroactively accepted"}
    ...
    write_new(P / "freeze_receipt.json", {"schema": "r32-paired-freeze-receipt/v1", "created_utc": now(),
...
if __name__ == "__main__":
    ...
    try:
        ...
        write_new(P / ("preparation_rejected.%04d.json" % (len(list(P.glob("preparation_rejected.*.json"))) + 1)),
            {"schema": "r32-preparation-failure/v1", "created_utc": now(), "reviewed_commit": args.reviewed_commit,
