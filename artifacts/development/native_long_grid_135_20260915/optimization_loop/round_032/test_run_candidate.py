...
spec = importlib.util.spec_from_file_location("r32_runner_tests", Path(__file__).with_name("run_candidate.py"))
...
def test_bad_barrier_count_cannot_unlock_scoring(tmp_path, monkeypatch):
    monkeypatch.setattr(d, "P", tmp_path)
    (tmp_path / "predictions_complete.json").write_text(json.dumps({"schema": "r32-full262-terminal-barrier/v1", "terminal_count": 131, "arms": {"off": {}}}))
