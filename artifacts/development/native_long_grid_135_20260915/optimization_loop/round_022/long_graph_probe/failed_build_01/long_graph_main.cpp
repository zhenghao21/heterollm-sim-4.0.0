...
static std::string setup_json(const Options &options, const ProbeConfig &config, const Resources &resources,
    ...
    output << "{\"record\":\"setup\",\"config\":" << quote(options.config) << ",\"arm\":" << quote(options.arm)
           ...
           << ",\"submission_entry\":\"ggml_backend_sched_graph_compute_async\",\"synchronization_entry\":\"ggml_backend_sched_synchronize\",\"scheduler_backends\":1,\"scheduler_parallel\":false,\"scheduler_op_offload\":false"
...
static RunSummary run_probe(const Options &options, Jsonl &raw, int argc, char **argv) {
    ...
    ggml_backend_t scheduler_backends[] = {resources.backend};
    resources.scheduler = ggml_backend_sched_new(scheduler_backends, nullptr, 1, capacity, false, false);
    if (!resources.scheduler || !ggml_backend_sched_alloc_graph(resources.scheduler, graph)) throw std::runtime_error("scheduler allocation failed");
