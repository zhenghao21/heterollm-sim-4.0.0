...
static CallSample timed_call(Harness &harness, const ArmContext &context, int ordinal) {
    ...
    sample.graph_status = static_cast<int>(ggml_backend_sched_graph_compute_async(harness.scheduler, harness.graph));
    ...
    ggml_backend_sched_synchronize(harness.scheduler); // Exactly one final sync for every whole-graph call in either arm.
    sample.sync_end = qpc();
    sync.close();
    outer.close();
