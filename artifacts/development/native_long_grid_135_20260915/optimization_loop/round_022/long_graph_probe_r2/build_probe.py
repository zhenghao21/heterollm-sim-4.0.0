"""Review-gated compile/freeze entry for the R22 long-graph probe. Never executes GPU work."""
...
SOURCE_FILES = [
    "long_graph_main.cpp", "long_graph_probe.cpp", "math_reference.h", "frozen_module_guard.h",
...
FORBIDDEN_BUILD_OUTPUTS = [
    "prepared_identity.h", "compile.cmd", "compile.log", "long-graph-probe.exe",
    "long-graph-probe.obj", "reference-host.exe", "reference-host.obj",
...
def main() -> None:
    ...
    if protocol["execution"]["configs"] != ["scale_f32_e262144_g64","scale_f32_e262144_g256"]:
        raise ValueError("complete fixed config domain required before compiling")
    ...
    if not options.compile:
        print(json.dumps({
            ...
            "configs": protocol["execution"]["configs"],
            "gpu_access": False,
        }))
        return
    ...
    cpu_lib = root / "source/llama.cpp-native-thread-control/build-native-thread-control/ggml/src/ggml-cpu.lib"
    common = [
    ...
    probe = common + [str(PACKAGE / "long_graph_main.cpp"), f"/Fe:{PACKAGE / 'long-graph-probe.exe'}", f"/Fo:{PACKAGE / 'long-graph-probe.obj'}", "/link", str(base_lib), str(cuda_lib), str(cpu_lib), toolchain["cuda_library"], "bcrypt.lib"]
    ...
    tools = [Path(toolchain["vcvars"]), compiler, compiler.parent / "link.exe", Path(cpu_lib), Path(toolchain["cuda_library"]), Path(protocol["profiler"]["path"])]
    ...
    files = dict(frozen)
    for path in [*map(Path, includes), PACKAGE / "prepared_identity.h", PACKAGE / "compile.log", PACKAGE / "long-graph-probe.exe", PACKAGE / "reference-host.exe", PACKAGE / "host_reference_validation.json"]:
        item = ref(path)
        files[str(item["path"])] = item
    verify(list(files.values()))
    manifest = {
        "schema": "long-graph-probe-build/v1",
        ...
        "executable": ref(PACKAGE / "long-graph-probe.exe"),
