...
static Options parse_options(int argc, char **argv) {
    ...
    const int selected = int(options.run) + int(options.smoke) + int(options.identity_check);
    if (selected != 1) throw std::runtime_error("select exactly one of --identity-check, --smoke, or --run");
    if (options.run && (options.config.empty() || options.arm.empty() || options.pair_id.empty() || options.output.empty())) {
        throw std::runtime_error("--run requires --config, --arm buffered, --pair-id and new --output");
    }
    if (options.run && options.arm != "buffered") throw std::runtime_error("R22 requires buffered arm");
    if (options.smoke && (options.config.empty() || options.output.empty() || !options.arm.empty() || !options.pair_id.empty())) {
        throw std::runtime_error("--smoke requires --config and new --output only");
    }
    if (options.smoke) options.arm = "smoke";
    return options;
}
...
static std::string cache_semantics_json() {
    return "{\"graph_constructed_once\":true,\"backend_buffer_allocated_once\":true,\"input_uploaded_once\":true,\"formal_call_reset\":false,\"cache_sweep\":false,\"arm_difference\":\"long_graph_nodes_only\"}";
}
...
static std::string setup_json(const Options &options, const ProbeConfig &config, const Resources &resources,
    ...
    output << "{\"record\":\"setup\",\"config\":" << quote(options.config) << ",\"arm\":" << quote(options.arm)
           ...
           << ",\"submission_entry\":\"ggml_backend_sched_graph_compute_async\",\"synchronization_entry\":\"ggml_backend_sched_synchronize\",\"scheduler_backends\":2,\"scheduler_parallel\":false,\"scheduler_op_offload\":false,\"scheduler_backend_order\":[\"CUDA\",\"CPU\"],\"all_graph_tensors_explicitly_pinned_cuda\":true,\"cpu_backend_role\":\"scheduler_terminal_requirement_only\",\"cpu_scheduler_buffer_bytes\":0"
    ...
    for (std::size_t index = 0; index < stages.size(); ++index) {
        ...
        output << "{\"index\":" << index << ",\"name\":" << quote(ggml_get_name(tensor))
               << ",\"src0\":" << quote(ggml_get_name(tensor->src[0])) << ",\"operator\":\"SCALE\",\"dtype\":\"F32\",\"scale\":" << graph_gap_scale_at(int(index)+1) << ",\"ne\":["
...
static RunSummary run_probe(const Options &options, Jsonl &raw, int argc, char **argv) {
    ...
    if (!config) throw std::runtime_error("configuration outside two frozen long-graph combinations");
    ...
    resources.cpu_backend = ggml_backend_cpu_init();
    if (!resources.cpu_backend) throw std::runtime_error("CPU backend initialization failed");
    ...
    for (int stage = 0; stage < config->nodes; ++stage) {
        auto *node = ggml_scale(resources.context, previous, graph_gap_scale_at(stage + 1));
    ...
    ggml_backend_t scheduler_backends[] = {resources.backend, resources.cpu_backend};
    resources.scheduler = ggml_backend_sched_new(scheduler_backends, nullptr, 2, capacity, false, false);
    if (!resources.scheduler) throw std::runtime_error("scheduler initialization failed");
    // The CPU backend is mandatory scheduler tail priority in this locked GGML revision. All actual graph tensors are
    // explicitly pinned to CUDA before splitting; CPU is present only to satisfy the scheduler contract and must remain idle.
    ggml_backend_sched_set_tensor_backend(resources.scheduler, input_tensor, resources.backend);
    for (auto *stage : stages) ggml_backend_sched_set_tensor_backend(resources.scheduler, stage, resources.backend);
    if (!ggml_backend_sched_alloc_graph(resources.scheduler, graph)) throw std::runtime_error("scheduler allocation failed");
    if (ggml_backend_sched_get_n_backends(resources.scheduler) != 2 ||
        ggml_backend_sched_get_backend(resources.scheduler, 0) != resources.backend ||
        ggml_backend_sched_get_backend(resources.scheduler, 1) != resources.cpu_backend ||
        ggml_backend_sched_get_n_splits(resources.scheduler) != 1 ||
        ggml_backend_sched_get_buffer_size(resources.scheduler, resources.cpu_backend) != 0) {
        throw std::runtime_error("scheduler backend order, CUDA placement, split count, or CPU-idle allocation mismatch");
    }
    if (ggml_backend_sched_get_tensor_backend(resources.scheduler, input_tensor) != resources.backend) {
        throw std::runtime_error("scheduler moved graph input away from CUDA");
    }
    for (auto *stage : stages) {
        if (ggml_backend_sched_get_tensor_backend(resources.scheduler, stage) != resources.backend) {
            throw std::runtime_error("scheduler moved graph node away from CUDA");
        }
    }
    ...
    graph_gap_probe::Harness harness{resources.backend, resources.scheduler, graph, stages, input.size()};
    ...
    RunSummary summary;
    if (options.smoke) {
        // This intentionally has no per-call QPC or NVTX timing boundary: one numeric graph smoke only.
        const auto status = ggml_backend_sched_graph_compute_async(harness.scheduler, harness.graph);
        if (status != GGML_STATUS_SUCCESS) throw std::runtime_error("non-timed smoke graph compute failed");
        ggml_backend_sched_synchronize(harness.scheduler);
        const auto checks = graph_gap_probe::check_all_stages(harness, "smoke", 0);
        summary.arm.graph_calls = 1;
        summary.arm.graph_status_pass = true;
        graph_gap_probe::include_block(summary.arm, checks);
        raw.line("{\"record\":\"smoke\",\"execution_mode\":\"non_timed_numeric_smoke\",\"graph_calls\":1,\"formal_calls\":0,\"check\":" + graph_gap_probe::validation_json(context, checks) + "}");
        raw.durable();
    } else {
        summary.arm = graph_gap_probe::run_buffered_arm(harness, context, emit, [&raw] { raw.durable(); });
    }
    verify_files();
    verify_loaded_modules();
...
int main(int argc, char **argv) {
    ...
    try {
        ...
        raw->line("{\"record\":\"header\",\"schema\":\"graph-gap-probe/v1\",\"utc_start\":" + quote(utc())
                  ...
                  + ",\"argv\":" + argv_json(argc, argv) + ",\"execution_mode\":" + quote(options.smoke ? "non_timed_numeric_smoke" : "formal_buffered")
                  + ",\"probe_cuda_events\":false,\"timing_clock\":\"absolute_QPC\",\"trace_clock_subtraction_allowed\":false}");
        raw->durable();
