...
inline float graph_gap_reference_at(std::size_t i, int stage) {
    return static_cast<float>(std::ldexp(static_cast<double>(graph_gap_input_numerator(i)), -8 - (stage % 2)));
}
...
// Alternating powers of two avoid subnormal/zero degeneration at256 nodes.
inline float graph_gap_scale_at(int stage) { return stage % 2 ? 0.5f : 2.0f; }
