"""Presentation-only rendering of immutable R22 engine error scores; no scoring/refit."""
...
def main():
 a=argparse.ArgumentParser(description=__doc__);a.add_argument('--score',type=Path,default=ROOT/'cta_issue/errors.0002.json');a.add_argument('--output',type=Path,default=ROOT/'full_engine_error_heatmap.png');args=a.parse_args()
 ...
 fig.suptitle('R22 conversion CTA + conditional MMVQ issue candidate | Fixed native development set | Engine absolute relative error (%)',fontsize=15,y=.995)
 ...
 fig.text(.045,.012,'Fixed set: 131 cells from 162 planned cells.  - = outside fixed set; X = failed/unscored.\nConditional MMVQ dot issue lower bound; unpriced work and retained KV state limits remain. Independent acceptance (B): unvalidated. All failures remain in denominators.',fontsize=10,color='#354557')
