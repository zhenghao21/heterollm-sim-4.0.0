...
PROBE=P.parent/'long_graph_probe_r2'
def manifest():return raw.load(PROBE/'build_manifest.json')
def protocol():return raw.load(PROBE/'protocol.json')

def actual_smoke_setup():
    rows=[json.loads(line) for line in (PROBE/'smoke_raw.jsonl').read_text(encoding='utf-8-sig').splitlines() if line.strip()]
    setups=[row for row in rows if row.get('record')=='setup']
    assert len(setups)==1
    return copy.deepcopy(setups[0])

R2_SCHEDULER_EXPECTED={
    'scheduler_backends':2,
    'scheduler_backend_order':['CUDA','CPU'],
    'all_graph_tensors_explicitly_pinned_cuda':True,
    'cpu_backend_role':'scheduler_terminal_requirement_only',
    'cpu_scheduler_buffer_bytes':0,
    'scheduler_parallel':False,
    'scheduler_op_offload':False,
}
...
def test_recorded_r2_smoke_setup_transforms_into_buffered_raw_contract(tmp_path):
    smoke=actual_smoke_setup()
    assert {key:smoke.get(key) for key in R2_SCHEDULER_EXPECTED}==R2_SCHEDULER_EXPECTED
    path,stage,m,p,rows=raw_fixture(tmp_path,64)
    assert {key:rows[1].get(key) for key in R2_SCHEDULER_EXPECTED}=={key:smoke.get(key) for key in R2_SCHEDULER_EXPECTED}
    assert raw.audit(path,stage,m,p,12)['math_pass'] is True

@pytest.mark.parametrize('mutation',[    'backend_count','backend_order','cuda_pin','cpu_role','cpu_buffer','parallel','op_offload',
])
def test_r2_scheduler_contract_component_tamper_is_rejected(tmp_path,mutation):
    path,stage,m,p,rows=raw_fixture(tmp_path,64);setup=rows[1]
    if mutation=='backend_count':setup['scheduler_backends']=1
    elif mutation=='backend_order':setup['scheduler_backend_order']=['CPU','CUDA']
    elif mutation=='cuda_pin':setup['all_graph_tensors_explicitly_pinned_cuda']=False
    elif mutation=='cpu_role':setup['cpu_backend_role']='fallback_compute'
    elif mutation=='cpu_buffer':setup['cpu_scheduler_buffer_bytes']=1
    elif mutation=='parallel':setup['scheduler_parallel']=True
    else:setup['scheduler_op_offload']=True
    save(path,rows)
