# R22 long synthetic graph collection r2
...
- `../long_graph_probe_r2/identity_check.py` is the environment-complete identity-only entry. The frozen invoke.ps1 helper only changes PATH and expects its caller to have already applied protocol environment; its initial environment rejection is retained in failed_identity_01.json. Use the Python entry for self-contained identity checks.
...
R2 binds only `../long_graph_probe_r2`: scheduler order is CUDA then CPU as required by the locked GGML scheduler. The r2 probe explicitly pins all graph tensors to CUDA and rejects any CPU graph allocation or placement before a formal run. The CPU backend is a terminal scheduler requirement, not accepted graph execution; CUPTI remains required to observe CUDA kernel and stream identity.

The r2 raw auditor now requires the exact two-backend contract: backend count2, ordered `[CUDA, CPU]`, explicit CUDA graph-tensor pin, terminal-only CPU role, zero CPU scheduler buffer, `parallel=false`, and `op_offload=false`. Its regression fixture starts from the recorded r2 smoke setup and separately rejects tampering with every scheduler-contract field. This corrects the earlier r1-only `scheduler_backends==1` audit mismatch. 38 mock/read-only tests passed; no execution freeze or formal stage has been created.
