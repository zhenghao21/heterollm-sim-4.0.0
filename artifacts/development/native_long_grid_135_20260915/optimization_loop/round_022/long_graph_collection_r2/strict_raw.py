"""R22 buffered long-graph raw evidence contract. No native calls, no cost fitting."""
...
def audit(path, stage, manifest, protocol, process_pid=None, extra_runtime_refs=()):
    ...
    for i,node in enumerate(nodes):
        require(node=={'index':i,'name':f'scale_{i+1}','src0':('graph_input' if i==0 else f'scale_{i}'),'operator':'SCALE','dtype':'F32','scale':(0.5 if (i+1)%2 else 2.0),'ne':[e,1,1,1]},'source dependency chain')
    ...
    started=one('buffered_started')
    require(started.get('arm')=='buffered' and started.get('config')==config['id'] and started.get('first_timed_ordinal')==0 and rows.index(started)<rows.index(calls[0]),'durable buffered start identity')
    require(not any(x.get('record')=='settle_metadata' for x in rows),'R22 forbids settle work metadata')
    require(a.get('write_cadence')=='buffered_after_postformal','buffered write cadence')
    require(s.get('submission_entry')=='ggml_backend_sched_graph_compute_async' and s.get('synchronization_entry')=='ggml_backend_sched_synchronize','scheduler whole-graph entry identity')
    require(s.get('scheduler_backends')==2 and s.get('scheduler_backend_order')==['CUDA','CPU'],'r2 scheduler backend count/order')
    require(s.get('all_graph_tensors_explicitly_pinned_cuda') is True,'r2 CUDA graph tensor pin')
    require(s.get('cpu_backend_role')=='scheduler_terminal_requirement_only' and s.get('cpu_scheduler_buffer_bytes')==0,'r2 CPU terminal-only scheduler allocation')
    require(s.get('scheduler_parallel') is False and s.get('scheduler_op_offload') is False,'r2 scheduler parallel/offload')
    ...
    return {'raw_ref':actual,'sidecar_ref':ref(str(path)+'.receipt.json'),'header':h,'setup':s,'footer':f,'calls':calls,'numeric_blocks':blocks,'math_pass':math_pass,'per_call_final_validated':False,'condition':stage.get('condition'),'host_formal':host,'formal_clock_intervals':[{'index':x['index'],'qpc_start':x['qpc_outer_push_start'],'qpc_end':x['qpc_outer_pop_end']} for x in formal]}