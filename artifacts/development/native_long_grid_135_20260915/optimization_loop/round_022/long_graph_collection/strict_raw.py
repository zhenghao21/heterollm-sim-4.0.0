...
def audit(path, stage, manifest, protocol, process_pid=None, extra_runtime_refs=()):
    ...
    require(s.get('submission_entry')=='ggml_backend_sched_graph_compute_async' and s.get('synchronization_entry')=='ggml_backend_sched_synchronize' and s.get('scheduler_backends')==1 and s.get('scheduler_parallel') is False and s.get('scheduler_op_offload') is False,'scheduler whole-graph entry identity')
