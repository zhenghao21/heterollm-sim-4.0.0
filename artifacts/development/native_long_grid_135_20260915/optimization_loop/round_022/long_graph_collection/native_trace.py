...
R22 preserves launch/kernel correlation and reports observed counts without substituting planned nodes.
"""
...
def validate_kernel_chain(kernels,config):
    ...
    if not ordered:issues.append('actual_device_work_missing')
    for k in ordered:
    ...
    return {'complete':not issues,'observed_family':'SCALE_F32' if not issues else 'unsupported',
        ...
        'planned_ggml_nodes':config['nodes'],'observed_count_matches_nodes':len(ordered)==config['nodes'],
        'fusion_status':'observed_SCALE_path_count_matches_nodes_no_fusion_inferred' if len(ordered)==config['nodes'] else 'observed_count_differs_fusion_not_proven',
        'fusion_proven':False,'dependency_policy':'Raw GGML predecessor-chain and all-stage numerical evidence plus actual correlated single-stream dispatch; neither PDL interval overlap nor count ratio proves fusion.'}
...
def correlate(raw,app,config):
    ...
    for marker in sorted(selected,key=lambda m:m['start']):
        ...
        calls.append({'phase':phase,'index':index,'nvtx_verbatim':marker,'api_events_verbatim':owned,'kernel_launch_pairs':pairs,
            ...
            'actual_kernel_count':len(mapped),'planned_GGML_nodes':config['nodes'],
            'launch_api_events_verbatim':launches,'graph_end_sync_api_events_verbatim':sync,
            'api_name_counts':dict(Counter(api_base(a.get('name_text')) for a in owned)),
            'CPU_driver_service_policy':'Retain actual API begin/end, globalTid, correlationId and kernel/stream pairs. Launch demand already exists in the simulator; no wall-minus-kernel latency constant and no coefficient fitting.','graph_launch_API_count':len(graph_launch),
    ...
    return {'schema':'graph-matrix-correlated-profile/v1','config':config,'calls':calls,'all36_chain_complete':complete,
        ...
        'outside_recorded_calls_policy':'All unmapped events are retained separately and never included in 36 mapped calls or formal30 estimates.',
