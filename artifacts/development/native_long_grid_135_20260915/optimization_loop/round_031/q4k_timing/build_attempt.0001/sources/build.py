...
SOURCES=('protocol.json','collector.cpp','build.py','runner.py','test_runner.py','wrapper_qualification.cpp','qualification.py','fixture_reference.h','fixture_lock.h','locked_identity.h','launch_decode.h','launch_metadata_json.h','test_qualification.py','README.md')
...
def host_test():
    ...
    with log.open('xb') as stream:r=subprocess.run([sys.executable,'-m','pytest','-q',str(P/'test_runner.py'),str(P/'test_qualification.py'),'--basetemp',str(P/'host_test_tmp')],cwd=ROOT,stdout=stream,stderr=subprocess.STDOUT)
