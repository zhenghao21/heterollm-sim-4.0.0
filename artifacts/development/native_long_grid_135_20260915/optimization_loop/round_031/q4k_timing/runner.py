...
now=b.now
natural_wait=b.load(P/"natural_wait.py","r31_shared_natural_wait")
...
def validate_activity(raw):
    ...
    for k in data['kernels']:
        ...
        geometry=protocol()['expected_conversion' if k['name']==CONV else 'expected_main']
        need(k['grid']==geometry['grid'] and k['block']==geometry['block'],'kernel geometry differs')
...
def wait_naturally(process,deadline_qpc_ns,directory,clock=time.perf_counter_ns):
    return natural_wait.wait_naturally(process,deadline_qpc_ns,directory,clock)
...
def run_process(directory,kind,start,proof=None,state=None,mode=None):
    ...
    try:
        ...
        need(not finish['observation_interrupted'],'observation interrupted; child exited naturally, stop parent phase')
...
def campaign(directory,phase,commit=None):
    ...
    directory=Path(directory).resolve();need(directory==P/'run.0001','R31 permits only its fixed run.0001 campaign')
    need(not (directory/'extension.json').exists(),'R31 extension evidence is outside the frozen budget')
    if phase=='run':
    ...
    try:
        ...
        for item in make_plan(n):
            ...
            if (folder/'finish.json').is_file():
                ...
                for r in b.references(finish):verify_ref(r)
                need(finish.get('observation_interrupted') is not True,'retained observation interruption stops campaign')
                continue
            ...
            need(result.get('observation_interrupted') is not True,'observation interrupted; current child exited naturally, stop new slots')
