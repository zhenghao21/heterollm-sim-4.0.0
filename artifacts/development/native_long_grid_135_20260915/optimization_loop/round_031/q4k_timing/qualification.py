...
need=b.need
ref=b.ref
natural_wait=b.load(P/"natural_wait.py","r31_qualification_natural_wait")
...
def execute():
    ...
    try:
        ...
        with (directory/'stdout.log').open('xb') as out,(directory/'stderr.log').open('xb') as err:
            ...
            result.update(natural_wait.wait_naturally(child,None,directory))
        need(not result['observation_interrupted'],'observation interrupted; wrapper child exited naturally, stop qualification')
