...
static constexpr const char *protocol_sha="1a118b535366c1959af876ac219bd91d0ec6ab16c61329f199e15ae3008639d6";
...
static constexpr const char *main_symbol="_Z13mul_mat_vec_qIL9ggml_type12ELi1ELb0ELb0ELb0EEvPKvS2_PKi31ggml_cuda_mm_fusion_args_devicePfj5uint3jjjS7_jjjS7_jjjj";
...
static J file_ref(const fs::path&p){return J{{"path",fs::absolute(p).string()},{"sha256",hash_file(p.string())},{"bytes",fs::file_size(p)}};}
static J read_json(const fs::path&p){std::ifstream in(p);need(bool(in),"JSON missing "+p.string());J j;in>>j;return j;}
static void verify_ref(const J&r){need(r.is_object()&&r.size()==3&&r==file_ref(r.at("path").get<std::string>()),"reference identity differs");}
...
static J modules(const J&p,bool traced){
 J rows=J::array();for(const auto&m:p.at("identities").at("native_modules")){verify_loaded(m.at("ref"),m.at("required"));if(auto h=GetModuleHandleA(m.at("name").get<std::string>().c_str()))rows.push_back(file_ref(module_path(h)));}
 for(auto key:{"CUPTI","nvperf_host","nvperf_target"}){const auto&r=p.at("identities").at(key);verify_loaded(r,traced&&std::string(key)=="CUPTI");if(auto h=GetModuleHandleA(fs::path(r.at("path").get<std::string>()).filename().string().c_str())){need(traced,"profiler DLL loaded in unobserved control");rows.push_back(file_ref(module_path(h)));}}
...
struct Trace{
 HMODULE dll=nullptr;CUpti_SubscriberHandle subscriber{};bool subscribed=false,enabled=false;uint32_t version=0;std::atomic<const char*> phase{"setup"};
 #define API(name) decltype(&name) name##_p=nullptr
 API(cuptiFinalize);API(cuptiGetVersion);API(cuptiSubscribe);API(cuptiUnsubscribe);API(cuptiEnableDomain);API(cuptiActivityEnableHWTrace);API(cuptiActivityRegisterCallbacks);API(cuptiActivityEnable);API(cuptiActivityDisable);API(cuptiActivityEnableRuntimeApi);API(cuptiActivityGetNextRecord);API(cuptiActivityGetNumDroppedRecords);API(cuptiActivityFlushAll);API(cuptiActivityPushExternalCorrelationId);API(cuptiActivityPopExternalCorrelationId);
 #undef API
 ...
 static void CUPTIAPI state_callback(void*,CUpti_CallbackDomain domain,CUpti_CallbackId id,const void*data){if(!live||domain!=CUPTI_CB_DOMAIN_STATE)return;try{const auto*s=static_cast<const CUpti_StateData*>(data);std::lock_guard<std::mutex>lock(live->mutex);live->states.push_back(J{{"id",id},{"result",int(s->notification.result)},{"message",s->notification.message?s->notification.message:""},{"phase",live->phase.load()},{"qpc",tick()}});}catch(...){live->overflow=true;}}
 ...
 void record(const char*name,CUptiResult status){calls.push_back(J{{"name",name},{"returncode",int(status)},{"phase",phase.load()},{"qpc",tick()}});}
 ...
 for(auto&b:buffers){if(!b.returned)continue;need(b.valid<=1048576,"invalid activity buffer length");const fs::path path=out/("activity_buffer."+std::to_string(index++)+".bin");write_bytes(path,b.data,b.valid);raw.push_back(J{{"ref",file_ref(path)},{"context",uintptr_t(b.context)},{"stream",b.stream},{"valid_bytes",b.valid}});
 ...
 bool state_clean(){std::lock_guard<std::mutex>lock(mutex);return states.empty()&&!overflow;}
...
struct Work{
 std::string role;bool pair=false;ggml_backend_t backend=nullptr;cudaStream_t stream=nullptr;std::vector<Slot>slots;std::vector<float>input;std::vector<uint8_t>packed,q8;check::Reference reference;
 Work(const std::string&role_,size_t count):role(role_),pair(role_=="pair"),input(check::make_input()),packed(check::packed_weight()),q8(check::expected_q8(input)),reference(check::reference(packed,q8)){
 ...
 J validate(const fs::path&out,const std::string&phase){J rows=J::array();bool all=true;std::vector<float>actual(check::N);std::vector<uint8_t>converted(q8.size());
  for(size_t i=0;i<slots.size();++i){auto&s=slots[i];if(role=="convert"){cuda_ok(cudaMemcpyAsync(converted.data(),s.dq,converted.size(),cudaMemcpyDeviceToHost,stream));sync();size_t bad=check::byte_mismatches(converted,q8);all&=bad==0;const auto path=out/(phase+"."+std::to_string(i)+".q8.bin");write_bytes(path,converted.data(),converted.size());rows.push_back(J{{"slot",i},{"byte_mismatches",bad},{"actual_ref",file_ref(path)}});}
   else {if(pair)ggml_backend_tensor_get(s.y,actual.data(),0,actual.size()*4);else{cuda_ok(cudaMemcpyAsync(actual.data(),s.dy,actual.size()*4,cudaMemcpyDeviceToHost,stream));sync();}auto v=check::compare_output(actual,reference);all&=v.failed==0;const auto path=out/(phase+"."+std::to_string(i)+".f32.bin");write_bytes(path,actual.data(),actual.size()*4);rows.push_back(J{{"slot",i},{"values",v.tested},{"failed",v.failed},{"max_absolute_error",std::isfinite(v.max_absolute_error)?J(v.max_absolute_error):J(nullptr)},{"max_bound_ratio",std::isfinite(v.max_bound_ratio)?J(v.max_bound_ratio):J(nullptr)},{"actual_ref",file_ref(path)}});}}
  return J{{"passed",all},{"rows",rows}};
 }
 J save_reference(const fs::path&out){J j;for(const auto&v:std::initializer_list<std::tuple<std::string,const void*,size_t>>{{"packed_q4",packed.data(),packed.size()},{"input_f32",input.data(),input.size()*4},{"expected_q8",q8.data(),q8.size()},{"reference_f64",reference.dot.data(),reference.dot.size()*8},{"bound_f64",reference.bound.data(),reference.bound.size()*8}}){auto path=out/(std::get<0>(v)+".bin");write_bytes(path,std::get<1>(v),std::get<2>(v));j[std::get<0>(v)]=file_ref(path);}return j;}
...
static void verify_capability(const fs::path&path,const J&p){
 const auto j=read_json(path);need(j.at("schema")=="r28-hes-api-contract/v2","legal HES capability receipt missing");const auto&r=j.at("capability_ref");verify_ref(r);const auto d=read_json(r.at("path").get<std::string>());
 need(d.at("protocol_ref")==file_ref(p.at("_protocol_path").get<std::string>()),"capability protocol differs");
 need(d.at("status")=="capability_observed_under_API_contract"&&d.at("GPU_kernel_executed")==false&&d.at("contexts_before").at("primary_active")==0&&d.at("contexts_before").at("current_context")==0,"capability boundary invalid");
 need(d.at("HES_enable_returncode")==0&&d.at("CUPTI_evidence").at("STATE").empty(),"HES SDK26 capability rejected");
}
static long long qpc_ns(){return static_cast<long long>(static_cast<long double>(tick())*1000000000.0L/frequency());}
int main(int argc,char**argv){
 std::map<std::string,std::string>a;for(int i=1;i+1<argc;i+=2)a[argv[i]]=argv[i+1];
 if(!std::getenv("GPU_OPERATOR_TIMING_AUTHORIZED")||std::string(std::getenv("GPU_OPERATOR_TIMING_AUTHORIZED"))!="1"||!a.count("--protocol")||!a.count("--output")||!a.count("--kind")||!a.count("--deadline-qpc-ns")){std::cerr<<"No GPU executed. Explicit root runner required.\n";return 2;}
 const fs::path out=a["--output"];if(!fs::is_directory(out)||fs::exists(out/"raw.json")){std::cerr<<"New prepared output directory required\n";return 2;}
 J result{{"schema","r28-gpu-operator-raw/v2"},{"status","rejected"},{"kind",a["--kind"]},{"timed",false},{"performance_parameter_admitted",false},{"service_cost_qualification","unvalidated"},{"QPC_frequency",frequency()},{"expected_CUPTI_API",26},{"budget_exceeded",false},{"budget_boundaries",J::array()}};
 Trace trace;bool traced=false;int exitcode=4;long long deadline=0;
 auto boundary=[&](const char*label){const auto t=qpc_ns();if(deadline>0&&t>=deadline){result["budget_exceeded"]=true;result["budget_boundaries"].push_back(J{{"phase",label},{"qpc_ns",t},{"deadline_qpc_ns",deadline},{"action","stop_new_calls_and_exit_naturally_after_synchronization"}});return true;}return false;};
 try{
 deadline=std::stoll(a["--deadline-qpc-ns"]);need(deadline>0,"positive absolute QPC deadline required");result["deadline_qpc_ns"]=deadline;need(!boundary("before_driver"),"budget exhausted before driver initialization");
 need(hash_file(a["--protocol"])==protocol_sha,"protocol identity differs");J p=read_json(a["--protocol"]);p["_protocol_path"]=a["--protocol"];result["protocol_ref"]=file_ref(a["--protocol"]);
 ...
 const bool capability=a["--kind"]=="capability_hes";traced=capability||(a.count("--mode")&&(a["--mode"]=="B"||a["--mode"]=="AB"));
 ...
 if(traced){trace.enable_hes();result["HES_enable_returncode"]=0;}else result["HES_enable_returncode"]=nullptr;
 result["actual_mode_evidence"]={{"kind",traced?"HWTrace_SDK26_API_contract_only":"unobserved_control"},{"direct_mode_readback_available",false},{"silent_fallback_independently_excluded",false},{"latency_timestamps_requested",false},{"software_fallback_requested",false}};
 if(capability){result["contexts_after"]=context_state();need(result["contexts_after"]["current_context"]==0&&result["contexts_after"]["primary_active"]==0,"capability unexpectedly created context");need(trace.state_clean(),"STATE notification in HES capability");result["modules_after"]=modules(p,true);need(!boundary("capability_complete"),"capability completed beyond budget");result["status"]="capability_observed_under_API_contract";result["GPU_kernel_executed"]=false;exitcode=0;}
 else{
 need(a["--kind"]=="formal"&&a.count("--state")&&a.count("--mode")&&a.count("--proof"),"formal arguments missing");const auto mode=a["--mode"];need(mode=="U"||mode=="A"||mode=="B"||mode=="AB","unknown mode");verify_capability(a["--proof"],p);result["mode_proof_ref"]=file_ref(a["--proof"]);
 ...
 const size_t weight_bytes=size_t(check::K)*check::N/256*144;const size_t L2=result["hardware"]["L2_bytes"].get<size_t>();need(L2>0,"L2 unavailable");const size_t count=state["cache"]=="rotation"?std::max<size_t>(2,(4*L2+weight_bytes-1)/weight_bytes+1):1;
 ...
 need(!boundary("before_work_setup"),"budget exhausted before work setup");Work w(state["role"],count);need(!boundary("after_setup_sync"),"budget exhausted after synchronized setup");result["references"]=w.save_reference(out);
 for(size_t i=0;i<count;++i){need(!boundary("before_correctness_call"),"budget exhausted before correctness call");w.invoke(i);w.sync();result["GPU_kernel_executed"]=true;need(!boundary("after_correctness_sync"),"budget exhausted after correctness call");}
 result["correctness_before"]=w.validate(out,"before");need(result["correctness_before"]["passed"],"pre-timing correctness failed");
 const auto warm_start=tick();size_t warm=0;do{need(!boundary("before_warmup_call"),"budget exhausted before warmup call");w.invoke(warm%count);w.sync();++warm;result["warmup"]={{"calls",warm},{"begin_qpc",warm_start},{"end_qpc",tick()}};need(!boundary("after_warmup_sync"),"budget exhausted at synchronized warmup boundary");need(ns(warm_start,tick())<=5e9,"warmup maximum exceeded");}while(warm<32||ns(warm_start,tick())<5e8);
 cudaEvent_t begin=nullptr,end=nullptr;const bool events=mode=="A"||mode=="AB";if(events){cuda_ok(cudaEventCreate(&begin));cuda_ok(cudaEventCreate(&end));}if(traced){need(trace.state_clean(),"STATE notification before formal");trace.begin();}
 const auto prime_begin=tick();size_t primes=0;for(int i=0;i<32;++i){if(boundary("before_observer_priming_call"))break;if(traced)trace.push(1000000+uint64_t(i));if(events)w.event(begin);w.invoke((warm+size_t(i))%count);if(events)w.event(end);w.sync();if(traced)trace.pop(1000000+uint64_t(i));++primes;if(boundary("after_observer_priming_sync"))break;}
 result["observer_priming"]={{"calls",primes},{"begin_qpc",prime_begin},{"end_qpc",tick()},{"external_base",1000000}};const size_t base_slot=(warm+primes)%count;result["formal_base_slot"]=base_slot;result["samples"]=J::array();result["formal_begin_qpc"]=tick();result["timed"]=true;
 if(!result["budget_exceeded"].get<bool>())for(int i=0;i<64;++i){if(boundary("before_formal_call"))break;const uint64_t id=uint64_t(i)+1;if(traced)trace.push(id);const auto start=tick();if(events)w.event(begin);const auto submit_begin=tick();w.invoke((base_slot+size_t(i))%count);const auto submit_end=tick();if(events)w.event(end);const auto wait_begin=tick();w.sync();const auto stop=tick();if(traced)trace.pop(id);
 J sample{{"sample",i},{"external_id",id},{"slot",(base_slot+size_t(i))%count},{"begin_qpc",start},{"end_qpc",stop},{"wall_ns",ns(start,stop)},{"submit_begin_qpc",submit_begin},{"submit_end_qpc",submit_end},{"wait_begin_qpc",wait_begin},{"event_ns",nullptr}};
 if(events){float ms=0;cuda_ok(cudaEventElapsedTime(&ms,begin,end));need(std::isfinite(ms)&&ms>0,"event envelope nonpositive");sample["event_ns"]=double(ms)*1e6;}result["samples"].push_back(sample);if(boundary("after_formal_sync"))break;}
 result["formal_end_qpc"]=tick();if(traced)result["activity"]=trace.finish(out);else result["activity"]=nullptr;
 if(begin)cuda_ok(cudaEventDestroy(begin));if(end)cuda_ok(cudaEventDestroy(end));boundary("after_activity_flush");
 if(!result["budget_exceeded"].get<bool>()){result["correctness_after"]=w.validate(out,"after");need(result["correctness_after"]["passed"],"post-timing correctness failed");boundary("after_correctness_sync");}else result["correctness_after_skipped_due_budget"]=true;
 result["modules_after"]=modules(p,traced);if(traced)need(trace.state_clean(),"STATE/callback failure observed");
 if(result["budget_exceeded"].get<bool>()){result["status"]="budget_exceeded_observation_rejected";exitcode=4;}else{result["status"]="formal_observed_pending_independent_validation";exitcode=0;}
 }
