...
static std::string runtime_json(const Recorder & rec) {
    ...
    for(unsigned i=0;i<count;++i) {
        ...
        s<<"{\"api_name\":"<<quote(r.api_name)<<",\"classification\":"<<quote(kind)
         ...
         <<",\"after_launch_pair\":"<<(r.after_launch_pair?"true":"false")
         <<",\"exit_seen\":"<<(r.exit_seen?"true":"false")<<",\"return_code\":"<<r.return_code<<'}';
    }
    s<<']';return s.str();
}
...
int main(int argc, char ** argv) {
    ...
    void *dw=nullptr,*dx=nullptr,*dq=nullptr,*dy=nullptr;cudaStream_t stream=nullptr;
    ...
    try {
        ...
        if(!backend)throw std::runtime_error("CUDA backend identity loader unavailable");
        if(cudaStreamCreateWithFlags(&stream,cudaStreamNonBlocking)!=cudaSuccess)throw std::runtime_error("stream unavailable");
        ...
        auto cuda_check=[](cudaError_t status){if(status!=cudaSuccess)throw std::runtime_error(cudaGetErrorString(status));};
        cuda_check(cudaMalloc(&dw,packed.size()));cuda_check(cudaMalloc(&dx,raw_input.size()));
        cuda_check(cudaMalloc(&dq,size_t(kK/32)*36));cuda_check(cudaMalloc(&dy,size_t(kN)*4));
        cuda_check(cudaMemcpyAsync(dw,packed.data(),packed.size(),cudaMemcpyHostToDevice,stream));
        cuda_check(cudaMemcpyAsync(dx,input.data(),raw_input.size(),cudaMemcpyHostToDevice,stream));
        cuda_check(cudaStreamSynchronize(stream));tensors={uintptr_t(dw),uintptr_t(dx),uintptr_t(dy)};
        auto pair=[&](){heterollm_mmvq_probe_convert_q8_1(static_cast<const float*>(dx),dq,12,kK,1,kK,stream);
            heterollm_mmvq_probe_main(dw,12,dq,static_cast<float*>(dy),kK,kN,1,kK/32,stream);};
        pair();cuda_check(cudaStreamSynchronize(stream));rec.reset();
        check(api.enable_domain(1,subscriber,CUPTI_CB_DOMAIN_RUNTIME_API));enabled=true;
        executed=true;pair();cuda_check(cudaStreamSynchronize(stream));
        ...
        if(valid){
            ...
            if(cudaMemcpy(actual.data(),dy,actual.size()*sizeof(float),cudaMemcpyDeviceToHost)!=cudaSuccess)throw std::runtime_error("output readback failed");
    ...
    result<<"{\"schema\":\"r31-q4k-wrapper-launch-capture/v1\",\"status\":"<<quote(valid?"path_observed_pending_independent_numeric":"rejected")
