...
def test_second_timing_campaign_cannot_start(tmp_path):
    with pytest.raises(ValueError,match='fixed run.0001'):r.campaign(tmp_path/'run.0002','run')


def test_injected_extension_evidence_cannot_expand_budget(tmp_path,monkeypatch):
    monkeypatch.setattr(r,'P',tmp_path)
    d=tmp_path/'run.0001';d.mkdir();(d/'extension.json').write_text('{}')
    with pytest.raises(ValueError,match='outside the frozen budget'):r.campaign(d,'resume')


def test_interrupted_qualification_waits_for_child_before_rejecting(tmp_path,monkeypatch):
    protocol=q.load(P/'protocol.json');(tmp_path/'protocol.json').write_text(json.dumps(protocol))
    (tmp_path/'build_manifest.json').write_text('{}')
    (tmp_path/'host_test_receipt.json').write_text(json.dumps({'returncode':0,'inputs_unchanged':True,'manifest_ref':q.ref(tmp_path/'build_manifest.json')}))
    monkeypatch.setattr(q,'P',tmp_path)
    monkeypatch.setattr(q.b,'verify_build',lambda:{'qualification_executable':{'path':'synthetic-no-GPU'}})
    monkeypatch.setattr(q,'verify_target',lambda:None)
    monkeypatch.setattr(q,'idle',lambda:{'conflicts':[]})
    directory=tmp_path/'qualification.0001'
    class Child:
        pid=123;calls=0
        def wait(self,timeout):
            ...
            return 0
    child=Child();monkeypatch.setattr(q.subprocess,'Popen',lambda *a,**k:child)
    with pytest.raises(ValueError,match='wrapper rejected'):q.execute()
    result=q.load(directory/'finish.json')
    assert child.calls==2 and result['status']=='rejected' and result['natural_exit'] and result['observation_interrupted']
    assert result['termination_requested'] is False and not result.get('numeric_qualified',False)
