...
SOURCES=('protocol.json','collector.cpp','build.py','runner.py','test_runner.py','wrapper_qualification.cpp','qualification.py','natural_wait.py','fixture_reference.h','fixture_lock.h','locked_identity.h','launch_decode.h','launch_metadata_json.h','test_qualification.py','README.md')
...
def inputs():
    ...
    need(protocol['sampling']['maximum_extensions']==0 and protocol['sampling']['maximum_formal_processes']==100,'successor budget differs')
...
def command(args,stem,out,cwd=None):
    ...
    with log.open('xb') as stream:
        child=subprocess.Popen(['cmd.exe','/c',str(cmd)],cwd=cwd or out,stdout=stream,stderr=subprocess.STDOUT,creationflags=subprocess.CREATE_NO_WINDOW)
        code=child.wait()
    need(code==0,'static build failed; retained '+str(log))
...
def build():
    ...
    try:
        ...
        obj=out/'collector.obj';exe=out/'q4k-timing.exe';deps=out/'dependencies.json'
        ...
        # Reuse the exact same generic type12 wrapper/conversion/support objects for a separate untimed qualifier.
        qobj=out/'wrapper_qualification.obj';qexe=out/'q4k-wrapper-qualification.exe';qdeps=out/'qualification.dependencies.json'
        records.append(command(args+['/sourceDependencies',qdeps,'/c',P/'wrapper_qualification.cpp','/Fo'+str(qobj)],'qualification.compile',out))
        records.append(command([VC/'link.exe','/nologo','/OPT:REF','/OUT:'+str(qexe),qobj,ids['wrapper_object']['path'],ids['conversion_object']['path'],ids['support_object']['path'],
            '/LIBPATH:'+str(LIB),'/LIBPATH:'+str(LIB/'ggml-cuda'),'/LIBPATH:'+str(CUDA/'lib/x64'),'ggml-base.lib','ggml-cpu.lib','ggml.lib','ggml-cuda.lib','cudart.lib','cuda.lib','bcrypt.lib'],'qualification.link',out))
        qdirectory=out/'qualification_device_code';qdirectory.mkdir()
        records.append(command([CUDA/'bin/cuobjdump.exe','--extract-elf','all',qexe],'qualification.cubin',out,qdirectory))
        for name,pair in ids['golden_cubins'].items():
            matches=[f for f in qdirectory.glob('*.cubin') if ref(f)['sha256']==pair['target_cubin']['sha256']]
            need(len(matches)==1,'qualification cubin differs '+name)
            symbol=protocol['expected_main' if name=='mmvq_main' else 'expected_conversion']['symbol'].encode()
            need(symbol in matches[0].read_bytes(),'Q4 device symbol missing from exact golden cubin '+name)
            device[name]['qualification_ref']=ref(matches[0])
        need(inputs()==before,'build inputs changed')
        includes=sorted(set(json.loads(deps.read_text(encoding='utf-8-sig'))['Data']['Includes']+json.loads(qdeps.read_text(encoding='utf-8-sig'))['Data']['Includes']))
        manifest={'schema':'r28-gpu-timing-build/v1','created_utc':now(),'inputs':before,'headers':[ref(x) for x in includes],'executable':ref(exe),'qualification_executable':ref(qexe),'qualification_object':ref(qobj),'object':ref(obj),'device_identity':device,'records':records,'GPU_executed':False,'cost_parameters_emitted':False}
...
def host_test():
    ...
    with log.open('xb') as stream:r=subprocess.run([sys.executable,'-m','pytest','-q',str(P/'test_runner.py'),str(P/'test_qualification.py'),'--basetemp',str(P/('host_test_tmp.'+Path(m['executable']['path']).parent.name))],cwd=ROOT,stdout=stream,stderr=subprocess.STDOUT)
