...
目标DLL不重编：使用GGML backend为权重/输入/输出分配真实CUDA buffer；一次不计时warmup后捕获一次replay。捕获期间不允许额外分配/memcpy/memset，callback只复制host参数，绝不调用CUDA或读device memory。通过路径门后，在禁用callback并同步之后回读actual.q8_1.bin与全部actual.f32.bin。
...
E:\anaconda\python.exe build.py host-test --manifest build_manifest.0002.json
~~~

未来仅root串行，所有R26/identity/native/simulator/GPU任务自然完成、project idle后：
~~~powershell
E:\anaconda\python.exe run_probe.py --manifest build_manifest.0002.json --execute-untimed-qualification
~~~
...
本交付使用0002构建；0001源码/清单保留在prepared_revision.0001。0002额外绑定link.exe和Python解释器身份，使资格检查后的任何必需原始文件缺失、CUPTI捕获清理失败都成为最终拒收。当前不应执行旧0001。
