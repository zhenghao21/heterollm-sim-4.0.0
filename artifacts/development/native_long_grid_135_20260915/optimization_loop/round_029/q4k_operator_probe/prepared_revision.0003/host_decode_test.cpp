...
struct Fixture {
    ...
    Fixture() {
        ...
        m.stride_row_x = 8; m.stride_col_y = m.stride_channel_y = m.stride_sample_y = 64;
        m.stride_col_dst = m.stride_channel_dst = m.stride_sample_dst = kN;
        m.stride_channel_x = m.stride_sample_x = 16384;
    ...
    void emit(Recorder & rec, bool conversion, const char * custom_symbol = nullptr,
        ...
        launch.gridDim = conversion ? dim3(8,1,1) : dim3(kN,1,1);
...
static cudaLaunchConfig_t extended_config(bool conversion,cudaLaunchAttribute * attrs,unsigned count) {
    cudaLaunchConfig_t cfg{};cfg.gridDim=conversion?dim3(8,1,1):dim3(kN,1,1);
...
int main() {
    try {
        static_assert(GGML_TYPE_Q4_K == 12);
        ...
        for(const char *name:{"cudaMalloc", "cudaMallocAsync", "cudaMallocAsync_ptsz", "cudaMallocFromPoolAsync",
                             "cudaMallocManaged", "cudaMallocHost", "cudaMallocArray", "cudaMalloc3D", "cudaHostAlloc",
                             "cudaFree", "cudaFreeAsync", "cudaFreeAsync_ptsz", "cudaFreeHost", "cudaFreeArray"}) {
            f.pair(rec);runtime_pair(name);
            require(rec.memory_api_count==0 && rec.runtime_count==1 &&
                    rec.runtime_calls[0].classification==RuntimeClass::Allocation && !accepted(rec,f),
                    "captured allocation/free accepted");
        }
        for(const char *name:{"cudaStreamSynchronize", "cudaStreamSynchronize_ptsz", "cudaDeviceSynchronize", "cudaEventSynchronize"}) {
            f.pair(rec);runtime_pair(name);
            require(rec.memory_api_count==0 && rec.runtime_count==1 && accepted(rec,f), "allocation/sync misclassified");
        }
        passed("reject_all_captured_allocation_free_allow_terminal_sync");
        ...
        rec.reset();runtime_pair("cudaStreamSynchronize");f.emit(rec,true);f.emit(rec,false);
        require(!accepted(rec,f) && !rec.runtime_calls[0].after_launch_pair,"leading sync accepted");
        passed("reject_sync_before_conversion");
        rec.reset();f.emit(rec,true);runtime_pair("cudaStreamSynchronize_ptsz");f.emit(rec,false);
        require(!accepted(rec,f) && !rec.runtime_calls[0].after_launch_pair,"inter-kernel sync accepted");
        passed("reject_sync_between_conversion_main");
        rec.reset();f.emit(rec,true);f.emit(rec,false,nullptr,false,false);runtime_pair("cudaStreamSynchronize");
        rec.records[1].exit_seen=true;rec.records[1].return_code=0;
        require(!accepted(rec,f) && !rec.runtime_calls[0].after_launch_pair,"sync before main EXIT accepted");
        passed("reject_sync_before_main_exit");
        f=Fixture{};
        ...
        require(eq(rec.records[0].grid,8,1,1)&&eq(rec.records[0].block,256,1,1)&&rec.records[0].shared==0&&
                ...
                rec.records[1].main.stride_row_x==8,"ExC config/attrs/args not deeply copied");
        ...
        require(ex_json.find("\"grid\":[8,1,1]")!=std::string::npos&&
