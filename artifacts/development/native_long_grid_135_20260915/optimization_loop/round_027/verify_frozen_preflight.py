...
def check(freeze_path, arm):
    ...
    spec = importlib.util.spec_from_file_location("r27_frozen_api", root / "tools/predict_stable_native_dataset.py")
    ...
    for name, module in list(sys.modules.items()):
        if name == "tools" or name.startswith("tools.") or name == "heterollm_sim" or name.startswith("heterollm_sim."):
            ...
            if origin is not None and not Path(origin).resolve().is_relative_to(root):
                raise ValueError("preflight import escaped arm source: " + name)
    # Do not mock or replace this call: this is the actual resume-start verifier.
    api.verify_freeze_references(frozen)
    from tools.native_final_output_binding import verify_cell, CONFIG_KEYS
    cells = frozen["cells"]
    ...
    for cell in cells:
        ...
        elif "final_output_selection" in inputs or "final_output_selection_binding" in inputs:
            raise ValueError("off arm contains treatment input")
    if reference(freeze_path) != before:
        raise ValueError("freeze changed during preflight")
    return {"verified_cells": len(cells), "source_sha256": frozen["source"]["sha256"],
        "selection_sha256": frozen["selection_sha256"], "qualification_counts": statuses,
        "actual_api": reference(root / "tools/predict_stable_native_dataset.py"),
        "actual_binding_helper": reference(root / "tools/native_final_output_binding.py")}
...
def main(argv=None):
    ...
    result = {"schema": "r27-frozen-source-preflight/v1", "stage": args.stage, "arm": args.arm,
