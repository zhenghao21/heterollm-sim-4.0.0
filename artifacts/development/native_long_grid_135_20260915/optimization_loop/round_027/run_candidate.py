"""R27 two-arm full262 terminal barrier; no scoring before both arms finish."""
...
spec = importlib.util.spec_from_file_location("r27_freezer", P / "freeze_candidate.py")
...
def command(arm, *args):
    subprocess.run([sys.executable, str(P / arm / "source/tools/predict_stable_native_dataset.py"),
        "--output", str(P / arm), *args], check=True)
...
def read_freeze_receipt():
    ...
    s.require(receipt.get("schema") == "r27-paired-freeze-receipt/v1" and receipt.get("compared_cells") == 131
    ...
    for arm in ARMS:
        ...
        f.check_source_inheritance(baseline, freezes[arm], protocol)
...
def guard():
    ...
    s.require(controls.get("schema") == "r27-two-arm-controls/v1" and controls.get("arms") == list(ARMS)
...
def lock():
    ...
    f.write_new(P / "controls.json", {"schema": "r27-two-arm-controls/v1", "created_utc": now(), "arms": list(ARMS),
...
def barrier():
    ...
    s.require(record.get("schema") == "r27-full262-terminal-barrier/v1" and record.get("terminal_count") == 262
...
def full(workers, timeout):
    ...
    f.write_new(P / "predictions_complete.json", {"schema": "r27-full262-terminal-barrier/v1", "created_utc": now(),
        "controls_ref": before, "arms": inputs, "terminal_count": 262, "failures_preserved": True,
        "target_errors_used_for_prediction": False, "old_R25_failed_attempt": "not overwritten or retroactively accepted"})
    barrier()
...
def score():
    ...
    if target.exists():
        ...
        return
    f.write_new(target, {"schema": "r27-two-arm-scoring-receipt/v1", "created_utc": now(), "barrier_ref": bref,
