"""Presentation/archive only, after existing R27 full262 barrier AND both scores.
Never predicts, scores, modifies native, or selects scenarios. Imports plotting
libraries only in the heatmaps action. Reuses R24 byte-preserving archive tools.
"""
...
ARCHIVE_HELPER_PINS = {
    "package_evidence.py": "d873b50609b0b9a1efee599463ef0e9b89591cdc7da83cee72baa5026093bde2",
    "split_evidence_packet.py": "e0f694666869c1032dab8c59bc75e5717aedb2299f129fb386271b3226e554bb",
...
def load_completed():
    ...
    r = load_module("r27_postprocess_runner", ROUND/"run_candidate.py")
    s = r.s
    barrier, barrier_ref = r.barrier()  # validates all 262 retained terminals/identities
    base, base_ref = s.read_json(ROUND/"report.json")
    require(base.get("schema") == "r27-two-arm-scoring-receipt/v1" and base.get("denominator_per_arm") == 131 and
            set(base.get("arms", {})) == {"off", "on"} and base.get("formal_success") is False and base.get("blind") is False,
            "unexpected completed two-arm scoring receipt")
    ...
    require(grouped.get("schema") == "r27-grouped-paired-errors/v1" and grouped.get("fixed_cells") == 131 and
    ...
    g = load_module("r27_postprocess_grouped", ROUND/"summarize_groups.py")
...
def heatmaps(context, output):
    ...
    ape_cmap = LinearSegmentedColormap.from_list("r27_error", [(0,"#e8f4ef"),(.09999,"#6bb69a"),(.1,"#fce4a3"),(.2,"#f3b15e"),(.5,"#d76c5e"),(1,"#913446")])
    ape_cmap.set_bad("#e4e7eb")
    delta_cmap = LinearSegmentedColormap.from_list("r27_delta", ["#17614e", "#f7f7f7", "#a63240"]); delta_cmap.set_bad("#e4e7eb")
    ...
    for view, groups in panels.items():
        ...
        fig.suptitle("R27 output-row selection | "+title,fontsize=15,y=.995)
    ...
    receipt={"schema":"r27-presentation-only-heatmaps/v1","created_utc":now(),"source_evidence":context["evidence"],"postprocessor_ref":ref(__file__),
             "fixed_cell_denominator":131,"fixed_metric_denominator_per_arm":393,"fixed_mask":sorted(context["cells"]),
...
def archive_commands(root):
    root = Path(root)
    packet,index,parts=root/"detailed_evidence.tar.gz",root/"detailed_evidence_index.json",root/"detailed_evidence.parts.json"
    return [[sys.executable,str(LEGACY/"package_evidence.py"),"--root",str(root)],
            [sys.executable,str(LEGACY/"split_evidence_packet.py"),"--packet",str(packet),"--index",str(index),"--output-dir",str(root),"--parts-index",str(parts)],
            [sys.executable,str(LEGACY/"restore_evidence.py"),"--root",str(root),"--packet",str(packet),"--index",str(index),"--parts-index",str(parts),"--verify-only"]]


def verify_parts(root):
    index=json.loads((root/"detailed_evidence_index.json").read_text(encoding="utf-8"))
    restore=load_module("r27_legacy_parts_verifier",LEGACY/"restore_evidence.py")
    _, rows=restore.load_parts_index(root/"detailed_evidence.parts.json",index)
    whole=hashlib.sha256();total=0;refs=[]
    for row in rows:
        path=root/row["filename"]; actual=ref(path)
        require(actual["bytes"]==row["bytes"] and actual["sha256"]==row["sha256"],"archive part changed")
        with path.open("rb") as stream:
            for block in iter(lambda:stream.read(1024*1024),b""):whole.update(block);total+=len(block)
        refs.append(actual)
    require(total==index["packet_bytes"] and whole.hexdigest()==index["packet_sha256"],"concatenated parts differ from original packet")
    return refs


def archive(context):
    helpers={name:ref(LEGACY/name) for name in ARCHIVE_HELPER_PINS}
    require(all(helpers[n]["sha256"]==sha for n,sha in ARCHIVE_HELPER_PINS.items()),"reviewed legacy archive helper changed")
    logdir=Path(__file__).resolve().parent/"archive.0001";logdir.mkdir(exist_ok=False)
    records=[]
    for step,command in enumerate(archive_commands(ROUND),1):
        check_unchanged(context["evidence"])
        with (logdir/f"step{step}.stdout.log").open("xb") as out,(logdir/f"step{step}.stderr.log").open("xb") as err:
            result=subprocess.run(command,stdout=out,stderr=err,check=False)
        record={"argv":command,"returncode":result.returncode,"stdout_ref":ref(logdir/f"step{step}.stdout.log"),"stderr_ref":ref(logdir/f"step{step}.stderr.log")};records.append(record)
        write_new(logdir/f"step{step}.json",record)
        require(result.returncode==0,"archive helper failed; partial exclusive artifacts retained")
    parts=verify_parts(ROUND);check_unchanged(context["evidence"])
    require(all(ref(LEGACY/n)==reference for n,reference in helpers.items()),"archive helper changed during execution")
    receipt={"schema":"r27-existing-tool-evidence-archive/v1","created_utc":now(),"source_evidence":context["evidence"],"helper_refs":helpers,
             "steps":records,"packet_ref":ref(ROUND/"detailed_evidence.tar.gz"),"index_ref":ref(ROUND/"detailed_evidence_index.json"),
             "parts_index_ref":ref(ROUND/"detailed_evidence.parts.json"),"part_refs":parts,"concatenated_parts_verified":True,
             "originals_modified_or_deleted":False,"copied_source_trees":False,
             "selection_policy":"unchanged R24: all prediction/freeze JSON, raw traces/JSONL, JSON >180 KiB; no source snapshots/binaries. Small reports and figures remain direct artifacts outside packet."}
    write_new(logdir/"archive.provenance.json",receipt)
    return {"part_count":len(parts),"provenance_ref":ref(logdir/"archive.provenance.json")}


def main():
    parser=argparse.ArgumentParser(description=__doc__);parser.add_argument("action",choices=("check","heatmaps","archive","all"))
    parser.add_argument("--figure-dir",type=Path,default=ROUND/"heatmaps.0001");args=parser.parse_args()
    context=load_completed();results={"completed_inputs_verified":True,"prediction_or_scoring_executed":False}
    if args.action in {"heatmaps","all"}:results["heatmaps"]=heatmaps(context,args.figure_dir)
    if args.action in {"archive","all"}:results["archive"]=archive(context)
