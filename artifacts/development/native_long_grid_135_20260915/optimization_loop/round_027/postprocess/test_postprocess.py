...
spec=importlib.util.spec_from_file_location("r27_postprocess_tests",Path(__file__).with_name("postprocess.py"))
...
def test_archive_commands_reuse_legacy_byte_preserving_tools_without_scoring(tmp_path):
    commands=p.archive_commands(tmp_path)
    assert [Path(c[1]).name for c in commands]==["package_evidence.py","split_evidence_packet.py","restore_evidence.py"]
    assert all(Path(c[1]).parent==p.LEGACY for c in commands)
    assert "--verify-only" in commands[-1]
    assert all("--score" not in c and "--resume" not in c for c in commands)
    assert commands[0][-1]==str(tmp_path)


def test_legacy_policy_keeps_small_failed_predictions_and_freezes():
    # Imports definitions only; no snapshot/read/package of current evidence.
    sys.path.insert(0,str(p.LEGACY))
    try:legacy=p.load_module("r27_test_legacy_packet",p.LEGACY/"package_evidence.py")
    finally:sys.path.pop(0)
    assert legacy.reason_for(Path("off/predictions/synthetic.failed.prediction.json"),10)=="prediction_result"
    assert legacy.reason_for(Path("on/freeze.json"),10)=="frozen_prediction_inputs"
    assert legacy.reason_for(Path("source/copied_binary.dll"),999999) is None
    assert legacy.reason_for(Path("small_report.json"),100) is None
    assert legacy.reason_for(Path("large_report.json"),180*1024+1)=="detailed_json_above_180_kib"
