"""Explicit-reference R27 packet. No recursive campaign discovery, score or prediction.
...
def select(root,heatmap_dir='heatmaps.0001'):
    root=Path(root).resolve();s=Selection(root)
    # Exact entry filenames only. Never discover evidence by its extension/name anywhere in tree.
    barrier=s.add(root/'predictions_complete.json','full262 barrier')
    need(barrier['schema']=='r27-full262-terminal-barrier/v1' and barrier['terminal_count']==262
    ...
    need(report['schema']=='r27-two-arm-scoring-receipt/v1' and report['denominator_per_arm']==131,'completed score receipt required')
    ...
    need(grouped['schema']=='r27-grouped-paired-errors/v1' and grouped['fixed_cells']==131,'grouped report incomplete')
    ...
    for arm in ('off','on'):
        ...
        for ident in ids:
            ...
            prediction=s.bound(entry['prediction_refs'][ident],directory/'predictions'/(ident+'.prediction.json'),'barrier terminal prediction')
        ...
        need(score_path.parent.resolve()==directory and re.fullmatch(r'errors\.[0-9]{4}\.json',score_path.name),'noncanonical score path')
        ...
        for stage,mapping in [('freeze',receipt['frozen_source_preflights']),('lock',controls['lock_preflights'])]:
            ...
            need(pre['schema']=='r27-frozen-source-preflight/v1' and pre['stage']==stage and pre['arm']==arm
        ...
        # Only direct run receipts with exact run.NNNN naming; rows must bind to barrier references.
        runfiles=sorted((directory/'runs').glob('run.[0-9][0-9][0-9][0-9].finish.json'))
        need(runfiles,'completed run receipt missing')
        covered=set()
        for finish_path in runfiles:
            run=s.add(finish_path,'actual arm run finish')
            runid=finish_path.name.removesuffix('.finish.json')
            need(run['schema']=='stable-native-prediction-run/v1' and run['phase']=='finish' and run['run_id']==runid,'run identity mismatch')
            ...
            need(scheduled<=set(ids),'run schedules unfrozen cells')
            runids=[]
            for row in run['cells']:
                ident=row['cell_id'];need(ident in scheduled,'run result not scheduled')
                need(normalize(row['prediction_ref'])==normalize(entry['prediction_refs'][ident]),'run prediction differs from barrier')
                runids.append(ident)
            need(len(set(runids))==len(runids) and set(runids)==scheduled,'run coverage mismatch')
            for ident in sorted(scheduled):s.add(directory/'predictions'/(ident+'.'+runid+'.worker.log'),'actual scheduled worker log')
    ...
    need(heat['schema']=='r27-presentation-only-heatmaps/v1' and heat['fixed_cell_denominator']==131
...
def assert_previous_idle():
    ...
    for process in psutil.process_iter(['pid','name','cmdline']):
        ...
        if row['pid']==70249 or ((row.get('name') or '').lower().startswith('python') and
...
def package_selected(root,output,members,part_size=40*1024*1024):
    ...
    finish={'schema':'r27-reference-selected-archive/v1','status':'rejected','created_utc':now(),
            'old_archive_status':'not_for_publication','old_archive_modified':False,'originals_modified':False,
    ...
    try:
        ...
        document={'schema':restore.SCHEMA,'created_utc':now(),'packet_name':packet.name,'packet_bytes':size,
                  ...
                  'selection_policy':finish['selection'],'identity_diagnostic_included':False}
...
def main():
    ...
    assert_previous_idle()
    result=package_selected(ROUND,output,members)
    print(json.dumps({'status':result['status'],'members':result['member_count'],'output':str(output),
                      'publication_files':result['publication_files'],'old_archive':'not_for_publication'}))
