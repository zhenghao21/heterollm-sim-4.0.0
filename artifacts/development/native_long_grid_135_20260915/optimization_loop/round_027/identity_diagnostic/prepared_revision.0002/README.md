...
~~~powershell
E:\anaconda\python.exe diagnostic.py run --manifest preparation_manifest.0002.json --authorize-large-file-read
~~~
...
当前准备清单为 preparation_manifest.0002.json；初版 preparation_manifest.json 保留为历史。初版交付前核验发现 Windows 系统目录大小写（WINDOWS/Windows）的路径比较误报，后继版本仅规范路径比较，仍逐文件核对SHA。
