...
def prepare():
    ...
    if (HERE/'protocol.json').exists():
        ...
        if not dh.same_file_stat(previous['target_stat_at_prepare'],before):raise ValueError('Prepared target stat changed')
    else:
    ...
    dh.write_new(HERE/'preparation_manifest.0002.json',m)
