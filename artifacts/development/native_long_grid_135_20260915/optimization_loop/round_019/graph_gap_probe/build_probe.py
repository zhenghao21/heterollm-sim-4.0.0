"""Review-gated compile/freeze entry for the R19 graph-gap probe. Never executes GPU work."""
...
SOURCE_FILES = [
    "graph_gap_main.cpp", "graph_gap_probe.cpp", "math_reference.h", "frozen_module_guard.h",
...
FORBIDDEN_BUILD_OUTPUTS = [
    "prepared_identity.h", "compile.cmd", "compile.log", "graph-gap-probe.exe",
    "graph-gap-probe.obj", "reference-host.exe", "reference-host.obj",
...
def main() -> None:
    ...
    probe = common + [str(PACKAGE / "graph_gap_main.cpp"), f"/Fe:{PACKAGE / 'graph-gap-probe.exe'}", f"/Fo:{PACKAGE / 'graph-gap-probe.obj'}", "/link", str(base_lib), str(cuda_lib), toolchain["cuda_library"], "bcrypt.lib"]
    ...
    files = dict(frozen)
    for path in [*map(Path, includes), PACKAGE / "prepared_identity.h", PACKAGE / "compile.log", PACKAGE / "graph-gap-probe.exe", PACKAGE / "reference-host.exe", PACKAGE / "host_reference_validation.json"]:
        item = ref(path)
        files[str(item["path"])] = item
    verify(list(files.values()))
    manifest = {
        "schema": "graph-gap-probe-build/v1",
        ...
        "executable": ref(PACKAGE / "graph-gap-probe.exe"),
