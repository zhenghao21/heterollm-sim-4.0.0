...
param(
 ...
 [Parameter(ParameterSetName='run',Mandatory=$true)][ValidateSet('control','buffered')][string]$Arm,
...
try {
 ...
 # This entry is deliberately limited to the protocol's preregistered pilot. Full-matrix execution needs a later reviewed amendment.
 $pilot=$protocol.execution.pilot
 if($pilot.config -cne 'scale_f32_e262144_g8' -or $Pair -gt [int]$pilot.pairs_per_arm){throw 'Run is outside the reviewed R19 pilot scope'}
 $targetArgs=@('--run','--config',$pilot.config,'--arm',$Arm,'--pair-id',"$($pilot.id).$Arm.pair$Pair",'--output',$Output)
 ...
 [pscustomobject]@{status=$receipt.status;pilot=$pilot.id;arm=$Arm;pair=$Pair;mode=$Mode;raw=$Output;calibration_ready=$false;trace_confirmation_required=$true}
