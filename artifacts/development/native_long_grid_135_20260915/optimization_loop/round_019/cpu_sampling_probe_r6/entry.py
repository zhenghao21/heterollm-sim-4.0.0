...
IDENTITY_FILE = P / 'source_identity_r4.json'
...
def compile_source(root_idle, root_authorized):
    ...
    build = {'schema': 'cpu-sampling-probe-build/v4', 'inputs': before, 'compiler': ref(CL), 'environment_setup': ref(VS), 'command': command, 'log': ref(log), 'exe': ref(exe), 'object': ref(obj), 'assembly_listing': ref(asm), 'compile_headers': [ref(path) for path in sorted(headers)], 'native_exports': pe, 'loader_binding': loader, 'probe_executed': False, 'native_DLL_rebuilt': False, 'candidate_binary_equivalence_proven': False}
...
def measure(args):
    ...
    destination.mkdir(exist_ok=False)
    plan = {'schema': 'cpu-sampling-probe-run-freeze/v4', 'build_manifest_ref': ref(P / 'build_manifest.json'), 'protocol_ref': ref(P / 'protocol.json'), 'actual_cpu_identity_freeze_ref': identity_ref, 'actual_cpu_identity': expected_identity, 'local_os_correlation_is_not_expected_hardware_identity': True, 'cpu_index': args.cpu_index, 'process_indices': [0, 1, 2], 'case_order': protocol['case_order'], 'holdout_vocabulary_sizes': [131072], 'allocation_and_reset_semantics': protocol['stages'], 'clock_policy': protocol['clock'], 'loops': {'warmup': 16, 'steady': 64}, 'loader_binding': loader, 'runtime_environment': {'LLAMA_TRACE_ANNOTATIONS': '0'}, 'root_authorized': True, 'compiled_schema_validation':schema_validation, 'no_auto_retry': True}
    ...
    for index in range(3):
        ...
        argv = [build['exe']['path'], '--measure-root-approved', '--native-dll', str(native), '--native-runtime-dir', loader['runtime_directory'], '--cuda-dependency-dir', loader['cuda_directory'], '--process-index', str(index), '--cpu-index', str(args.cpu_index), '--output', str(result)]
        child_environment = {**os.environ, 'LLAMA_TRACE_ANNOTATIONS': '0'}
        with stdout.open('xb') as out, stderr.open('xb') as err:
            completed = subprocess.run(argv, cwd=P, stdout=out, stderr=err, check=False, env=child_environment)
        receipt = {'argv': argv, 'explicit_environment': {'LLAMA_TRACE_ANNOTATIONS': '0'}, 'returncode': completed.returncode, 'stdout_ref': ref(stdout), 'stderr_ref': ref(stderr), 'run_freeze_ref': ref(destination / 'run_freeze.json'), 'result_ref': ref(result) if result.exists() else {'status': 'missing'}}
