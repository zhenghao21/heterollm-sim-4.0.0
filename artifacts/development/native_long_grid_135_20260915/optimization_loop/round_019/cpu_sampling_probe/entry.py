...
IDENTITY_FILE = P / 'source_identity_r2.json'
...
def inputs():
    ...
    native = next(Path(item['path']) for item in identity['native_runtime_modules'] if Path(item['path']).name == 'llama.dll')
    ...
    source = (P / 'cpu_sampling_probe.cpp').read_text(encoding='utf-8')
    ...
    for literal in ('VOCABS{32768, 131072, 262144}', 'WARMUP_CALLS = 16, STEADY_REPEATS = 64, PROCESS_COUNT = 3', 'INPUT_SEED = 20260916', 'cur_p = { cur.data(), cur.size(), -1, false };'):
        if literal not in source:
            raise ValueError('source/protocol constant mismatch')
    if protocol['vocabulary_sizes'] != [32768, 131072, 262144] or protocol['processes'] != 3 or protocol['steady_raw_repeats_per_stage_case_process'] != 64:
        raise ValueError('protocol domain changed')
    refs.extend(ref(P / name) for name in ('cpu_sampling_probe.cpp', 'entry.py', 'protocol.json', IDENTITY_FILE.name))
    return identity, protocol, list({item['path']: item for item in refs}.values()), pe, native
...
def compile_source(root_idle, root_authorized):
    ...
    identity, protocol, before, pe, native = inputs()
    ...
    exe, obj, asm = P / f'cpu-sampling-probe.{attempt:04d}.exe', P / f'cpu-sampling-probe.{attempt:04d}.obj', P / f'cpu-sampling-probe.{attempt:04d}.asm'
    if any(path.exists() for path in (exe, obj, asm)):
        raise ValueError('refuse existing compile attempt')
    command = '"' + str(CL) + '" /nologo /showIncludes /std:c++17 /EHsc /O2 /fp:strict /MD /utf-8 /GL- /FAs ' + ' '.join('/I"' + str(path) + '"' for path in INCLUDES) + ' "' + str(P / 'cpu_sampling_probe.cpp') + '" /Fe:"' + str(exe) + '" /Fo:"' + str(obj) + '" /Fa"' + str(asm) + '" /link PowrProf.lib'
    cmd, log = P / (stem + '.cmd'), P / (stem + '.log')
    with cmd.open('x', encoding='utf-8') as handle:
        handle.write('@echo off\ncall "' + str(VS) + '" >nul\nif errorlevel 1 exit /b %errorlevel%\n' + command + '\n')
    with (P / (stem + '.source.cpp')).open('xb') as handle:
        handle.write((P / 'cpu_sampling_probe.cpp').read_bytes())
    ...
    build = {'schema': 'cpu-sampling-probe-build/v2', 'inputs': before, 'compiler': ref(CL), 'environment_setup': ref(VS), 'command': command, 'log': ref(log), 'exe': ref(exe), 'object': ref(obj), 'assembly_listing': ref(asm), 'compile_headers': [ref(path) for path in sorted(headers)], 'native_exports': pe, 'probe_executed': False, 'native_DLL_rebuilt': False, 'candidate_binary_equivalence_proven': False}
...
def verified_build():
    ...
    if build['inputs'] != before:
        raise ValueError('frozen probe build inputs changed')
    for key in ('compiler', 'environment_setup', 'exe', 'object', 'assembly_listing'):
        verify(build[key])
    for item in build['compile_headers']:
        verify(item)
    return build, protocol, native
...
def validate_result(doc, index, cpu, expected_identity):
    ...
    for case in cases:
        if case['split'] != ('holdout' if case['vocabulary_size'] == 131072 else 'train'):
            raise ValueError('train/holdout drift')
        ...
        if len(case['stages']) != 2 or {stage['stage'] for stage in case['stages']} != {'candidate_loop', 'original_dll_topk_apply'}:
            raise ValueError('independent stage records missing')
        case_key = (case['vocabulary_size'], case['pattern'])
    ...
    return {'stable_stages': stable_stages, 'total_stages': 12, 'stage_quality': stage_quality, 'case_process_records': 6}
...
def capture_identity(args):
    ...
    build, protocol, native = verified_build()
    ...
    receipt = {'argv': argv, 'returncode': completed.returncode, 'stdout_ref': ref(stdout), 'stderr_ref': ref(stderr), 'result_ref': ref(result) if result.exists() else {'status': 'missing'}}
    ...
    if document.get('schema') != 'cpu-sampling-probe-actual-cpu-identity/v1' or document.get('status') != 'complete' or document.get('GPU_context_created') is not False or document.get('model_loaded') is not False:
...
def read_identity_freeze(path, cpu):
    ...
    if document.get('schema') != 'cpu-sampling-probe-actual-cpu-freeze/v1' or document.get('cpu_index') != cpu:
...
def summarize_quality(results, identity_ref):
    if len(results) != 3 or len({item['document']['process_id'] for item in results}) != 3:
        raise ValueError('three independent process records required for quality')
    ...
    if total != 36 or len(stages) != 36 or case_process_total != 18:
        raise ValueError('all 36 stage records and 18 case-process records required for quality')
    groups = {}
    for stage in stages:
    ...
    return {'schema': 'cpu-sampling-probe-quality/v3', 'exact_numeric_and_structure_pass': True, 'process_count': 3, 'case_process_records_total': case_process_total, 'stage_records_total': total, 'steady_samples': 2304, 'identity_freeze_ref': identity_ref, 'identity_consistent': True, 'frequency_stable_stages': stable, 'frequency_changed_stages': total - stable, 'steady_dispersion_pass_stages': sum(stage['steady_dispersion_pass'] for stage in stages), 'observer_overhead_pass_stages': sum(stage['observer_overhead_pass'] for stage in stages), 'cross_process_case_stage_groups_total': len(cross_process), 'cross_process_case_stage_groups_pass': sum(item['pass'] for item in cross_process), 'cross_process_medians': cross_process, 'timing_usable': timing_usable, 'diagnostic_only': not timing_usable, 'accepted_for_timing_evidence': timing_usable, 'timing_quality_failure_reasons': reasons, 'fit_performed': False, 'holdout_used_for_fit': False}
...
def measure(args):
    ...
    destination.mkdir(exist_ok=False)
    plan = {'schema': 'cpu-sampling-probe-run-freeze/v2', 'build_manifest_ref': ref(P / 'build_manifest.json'), 'protocol_ref': ref(P / 'protocol.json'), 'actual_cpu_identity_freeze_ref': identity_ref, 'actual_cpu_identity': expected_identity, 'local_os_correlation_is_not_expected_hardware_identity': True, 'cpu_index': args.cpu_index, 'process_indices': [0, 1, 2], 'case_order': protocol['case_order'], 'holdout_vocabulary_sizes': [131072], 'allocation_and_reset_semantics': protocol['stages'], 'clock_policy': protocol['clock'], 'loops': {'warmup': 16, 'steady': 64}, 'root_authorized': True, 'no_auto_retry': True}
    ...
    for index in range(3):
        ...
        argv = [build['exe']['path'], '--measure-root-approved', '--native-dll', str(native), '--process-index', str(index), '--cpu-index', str(args.cpu_index), '--output', str(result)]
        with stdout.open('xb') as out, stderr.open('xb') as err:
            completed = subprocess.run(argv, cwd=P, stdout=out, stderr=err, check=False, timeout=120)
        receipt = {'argv': argv, 'returncode': completed.returncode, 'stdout_ref': ref(stdout), 'stderr_ref': ref(stderr), 'run_freeze_ref': ref(destination / 'run_freeze.json'), 'result_ref': ref(result) if result.exists() else {'status': 'missing'}}
        ...
        document = json.loads(result.read_text(encoding='utf-8'))
        results.append({'document': document, 'frequency': validate_result(document, index, args.cpu_index, expected_identity)})
    if len({item['document']['process_id'] for item in results}) != 3:
        raise ValueError('independent process identity check failed')
    quality = summarize_quality(results, identity_ref)
    quality['result_refs'] = [ref(destination / f'process_{index}.json') for index in range(3)]
...
def main():
    ...
    parser.add_argument('mode', choices=('inspect-r2', 'compile', 'capture-identity', 'measure'))
    ...
    if args.mode == 'inspect-r2':
        identity, protocol, refs, pe, native = inputs()
        report = {'schema': 'cpu-sampling-probe-readonly-inspection/v2', 'source_equivalent_loop_statements_checked': True, 'native_PE': pe, 'input_refs': refs, 'identity_freeze_required_before_measure': True, 'all_frequency_fields_gate_timing_use': True, 'GPU_executed': False, 'DLL_loaded': False, 'compiled': False, 'probe_executed': False}
        write_new(P / 'readonly_inspection_r2.json', report)
