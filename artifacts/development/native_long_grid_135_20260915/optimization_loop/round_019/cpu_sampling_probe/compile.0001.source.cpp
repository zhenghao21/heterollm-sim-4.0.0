...
struct NativeSampler {
    ...
    explicit NativeSampler(const fs::path& path) {
        require(path.is_absolute() && path.filename() == L"llama.dll", "absolute original llama.dll path required");
        library = LoadLibraryExW(path.c_str(), nullptr, LOAD_LIBRARY_SEARCH_DLL_LOAD_DIR | LOAD_LIBRARY_SEARCH_DEFAULT_DIRS);
        require(library != nullptr, "cannot load selected native llama.dll");
        try {
            require(_wcsicmp(loaded_path(library).c_str(), path.lexically_normal().c_str()) == 0, "loaded llama.dll path mismatch");
            module_evidence = "[";
            for (const wchar_t* name : {L"llama.dll", L"ggml.dll", L"ggml-base.dll"}) {
                HMODULE module = GetModuleHandleW(name); require(module != nullptr, "native dependency is not loaded");
                const std::wstring actual = loaded_path(module);
                const fs::path expected = path.parent_path() / name;
                require(_wcsicmp(actual.c_str(), expected.lexically_normal().c_str()) == 0, "native dependency resolved outside locked directory");
    ...
    ~NativeSampler() { if (sampler) free_sampler(sampler); if (library) FreeLibrary(library); }
...
int wmain(int argc, wchar_t** argv) {
    try {
        fs::path dll, output; int process_index=-1, cpu=-1; bool approved=false, identity_only=false;
        for(int i=1;i<argc;++i) {
            ...
            if(option==L"--measure-root-approved" || option==L"--identity-root-approved") approved=true;
                ...
                if(option==L"--native-dll") dll=value; else if(option==L"--output") output=value;
        ...
        require(approved && process_index>=0 && process_index<int(PROCESS_COUNT) && cpu>=0 && cpu<64 && !dll.empty() && !output.empty(), "explicit root measurement approval, process-index 0..2, cpu-index, native-dll and output required");
        ...
        try {
            ...
            NativeSampler native(dll.lexically_normal());
            ...
            observer_ticks+=']';
            metadata="\"schema\":\"cpu-sampling-independent-probe/v1\",\"GPU_context_created\":false,\"model_loaded\":false,\"full_sampler_chain_measured\":false"
