...
def measure(args):
    ...
    destination.mkdir(exist_ok=False)
    plan = {'schema': 'cpu-sampling-probe-run-freeze/v4', 'build_manifest_ref': ref(P / 'build_manifest.json'), 'protocol_ref': ref(P / 'protocol.json'), 'actual_cpu_identity_freeze_ref': identity_ref, 'actual_cpu_identity': expected_identity, 'local_os_correlation_is_not_expected_hardware_identity': True, 'cpu_index': args.cpu_index, 'process_indices': [0, 1, 2], 'case_order': protocol['case_order'], 'holdout_vocabulary_sizes': [131072], 'allocation_and_reset_semantics': protocol['stages'], 'clock_policy': protocol['clock'], 'loops': {'warmup': 16, 'steady': 64}, 'loader_binding': loader, 'runtime_environment': {'LLAMA_TRACE_ANNOTATIONS': '0'}, 'root_authorized': True, 'no_auto_retry': True}
    ...
    for index in range(3):
        ...
        with stdout.open('xb') as out, stderr.open('xb') as err:
            completed = subprocess.run(argv, cwd=P, stdout=out, stderr=err, check=False, timeout=120, env=child_environment)
