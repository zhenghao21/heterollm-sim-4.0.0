...
def fixture(mode='event'):
    ...
    for phase,index in [('first_call',0)]+[('warmup',i) for i in range(5)]+[('formal',i) for i in range(30)]:
        row={'phase':phase,'index':index,'graph_computations':1024,'qpc_start':0,'qpc_record_begin_start':1,'qpc_record_begin_end':2,'qpc_submit_start':3,'qpc_submit_end':70,'qpc_record_end_start':71,'qpc_record_end_end':72,'qpc_wait_start':73,'qpc_wait_end':100,'qpc_end':100,'host_wall_ns':100000,'host_per_graph_ns':97.65625,'correctness':check,'event_envelope_ms':0.09 if mode=='event' else None,'cuda_query_before_wait':600}
