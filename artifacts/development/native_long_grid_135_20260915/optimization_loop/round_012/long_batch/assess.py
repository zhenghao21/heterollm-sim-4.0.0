...
def assess(event,control,config,protocol,manifest):
    ...
    for mode,data in [("event",event),("control",control)]:
        ...
        stats[mode]={"formal_batches":len(values),"graph_computations_per_batch":1024,"host_batch_median_ns":med,"host_per_graph_median_ns":med/1024,"host_p90_p10":spread,"host_max_deviation_from_median_fraction":max(abs(x/med-1) for x in values)}
        ...
        if mode=="event":
            ...
            if all(positive(x) for x in values):
                med=statistics.median(values);spread=percentile(values,.9)/percentile(values,.1)
                stats[mode].update(event_batch_median_ms=med,event_per_graph_median_ns=med*1e6/1024,event_p90_p10=spread,event_max_deviation_from_median_fraction=max(abs(x/med-1) for x in values))
