...
def validate_raw(r,mode,topology,sha):
 if mode=='path':
  ...
  base=HERE.parent/'host_submission_probe';spec=importlib.util.spec_from_file_location('host_service_predecessor_analyzer',base/'analyze.py');a=importlib.util.module_from_spec(spec);spec.loader.exec_module(a)
  ...
  a.validate_record(v,protocol,'path',topology,sha)
...
def execute(a):
 ...
 if finish.get('raw_validated') and finish['inputs_unchanged'] and finish['process_gates_passed'] and finish['campaign_gates_passed']:finish['status']='validated'
 for key,file in [('start_ref','start.json'),('child_ref','child.json'),('record_ref','record.json')]:finish[key]=build.ref(directory/file) if(directory/file).exists() else None
