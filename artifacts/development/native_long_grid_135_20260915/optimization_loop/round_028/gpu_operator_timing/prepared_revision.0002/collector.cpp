...
static void verify_capabilities(const fs::path&path,const J&p){const auto j=read_json(path);need(j.at("schema")=="r28-hes-counterfactual-proof/v1","counterfactual proof missing");J docs;for(auto key:{"without_hes","after_hes"}){const auto&r=j.at(key);verify_ref(r);docs[key]=read_json(r.at("path").get<std::string>());need(docs[key].at("protocol_ref")==file_ref(p.at("_protocol_path").get<std::string>()),"capability protocol differs");need(docs[key].at("status")=="capability_observed"&&docs[key].at("contexts_before").at("primary_active")==0&&docs[key].at("contexts_before").at("current_context")==0,"capability phase invalid");}
...
int main(int argc,char**argv){
 ...
 try{need(hash_file(a["--protocol"])==protocol_sha,"protocol identity differs");J p=read_json(a["--protocol"]);p["_protocol_path"]=a["--protocol"];result["protocol_ref"]=file_ref(a["--protocol"]);
 ...
 need(a["--kind"]=="formal"&&a.count("--state")&&a.count("--mode")&&a.count("--proof"),"formal arguments missing");const auto mode=a["--mode"];need(mode=="U"||mode=="A"||mode=="B"||mode=="AB","unknown mode");verify_capabilities(a["--proof"],p);result["mode_proof_ref"]=file_ref(a["--proof"]);if(traced)trace.enable_hes();
 ...
 if(begin)cuda_ok(cudaEventDestroy(begin));if(end)cuda_ok(cudaEventDestroy(end));result["correctness_after"]=w.validate(out,"after");need(result["correctness_after"]["passed"],"post-timing correctness failed");result["modules_after"]=modules(p,traced);if(traced)need(trace.state_clean(),"STATE/callback failure observed");result["status"]="formal_observed_pending_independent_validation";result["actual_mode_evidence"]={{"kind",traced?"HWTrace_API_success_bound_to_counterfactual_and_clean_STATE":"unobserved_control"},{"direct_mode_readback_available",false},{"latency_timestamps_requested_in_formal",false},{"software_fallback_allowed",false}};exitcode=0;
 }
