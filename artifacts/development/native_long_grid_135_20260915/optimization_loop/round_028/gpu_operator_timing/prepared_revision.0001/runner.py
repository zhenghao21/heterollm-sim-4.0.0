...
MAIN='_Z13mul_mat_vec_qIL9ggml_type6ELi1ELb0ELb0ELb0EEvPKvS2_PKi31ggml_cuda_mm_fusion_args_devicePfj5uint3jjjS7_jjjS7_jjjj'
...
def pair_metrics(kernels):
    ...
    return {'conversion_lifespan_ns':d0,'main_in_pair_lifespan_ns':d1,'pair_span_ns':span,'pair_union_ns':union,'interval_overlap_ns':overlap,'inter_kernel_gap_ns':span-union}

def validate_capability(raw,after_hes):
    need(raw.get('schema')=='r28-gpu-operator-raw/v1' and raw.get('status')=='capability_observed','capability did not finish')
    ...
    need(raw.get('kind')==('capability_after_hes' if after_hes else 'capability_without_hes'),'capability role wrong')
    ...
    need(type(raw.get('latency_api_returncode')) is int and raw['latency_api_returncode']==(27 if after_hes else 0),'same-stage counterfactual cannot prove HES; no silent fallback')
    need(raw.get('HES_enable_returncode')==(0 if after_hes else None),'wrong HES enable result')
    trace=raw['CUPTI_evidence'];need(trace.get('compile_API_version')==26 and trace.get('runtime_API_version')==26 and trace.get('callback_overflow') is False,'capability ABI/callback invalid')
    for notice in trace.get('STATE',[]):
        need(after_hes and notice.get('phase')=='counterfactual_latency_api_only' and notice.get('result')==27 and notice.get('id')==2,'unexpected STATE notification in mode proof')
    calls=trace.get('calls',[])
    latency=[c for c in calls if c['name']=='counterfactual_latency_enable(1)']
    need(len(latency)==1 and latency[0]['returncode']==(27 if after_hes else 0),'capability API evidence missing')
    if after_hes:
        ...
        need(raw['driver_init']['qpc_end']<=enables[0]['qpc']<=latency[0]['qpc'],'HES initialization ordering invalid')
    else:need(not any(c['name']=='cuptiActivityEnableHWTrace(1)' for c in calls),'software counterfactual enabled HES')
    return True

def validate_numeric(raw):
    refs=raw['references'];need(set(refs)=={'packed_q5','input_f32','expected_q8','reference_f64','bound_f64'},'reference fixture incomplete')
    for r in refs.values():verify_ref(r)
    expected_q8=Path(refs['expected_q8']['path']).read_bytes();need(len(expected_q8)==4608,'Q8 reference shape wrong')
    expected_data=Path(refs['reference_f64']['path']).read_bytes();bound_data=Path(refs['bound_f64']['path']).read_bytes()
    need(len(expected_data)==len(bound_data)==3072*8,'output reference dimensions wrong')
    expected=struct.unpack('<3072d',expected_data);bounds=struct.unpack('<3072d',bound_data)
    ...
    for phase in ('correctness_before','correctness_after'):
        ...
        for row in value['rows']:
            ...
            if role=='convert':need(data==expected_q8 and row.get('byte_mismatches')==0,'Q8 conversion differs')
                ...
                need(len(data)==3072*4 and row.get('values')==3072 and row.get('failed')==0,'output dimensions/results differ')
                actual=struct.unpack('<3072f',data);errors=[abs(a-e) for a,e in zip(actual,expected)]
...
def validate_activity(raw):
    ...
    for k in data['kernels']:
        ...
        shape=([16,1,1],[256,1,1]) if k['name']==CONV else ([3072,1,1],[32,4,1])
    ...
    for ident in list(range(1000000,1000032))+list(range(1,65)):
        ...
        if role=='pair':
            ...
            value['host_late_submission_lower_bound_ns']=min(value['inter_kernel_gap_ns'],max(0,api[main['correlation']]['start']-conversion['end']))
        else:value={'independent_main_ns' if role=='main' else 'independent_conversion_ns':rows[0]['end']-rows[0]['start']}
...
def validate_raw(raw,state_id,mode,verify_files=True):
    ...
    need(raw.get('status')=='formal_observed_pending_independent_validation' and raw.get('kind')=='formal' and raw.get('state')==state and raw.get('mode')==mode,'formal state/status differs')
    ...
    if traced:
        ...
        actual=raw['actual_mode_evidence'];need(actual.get('kind')=='HWTrace_API_success_bound_to_counterfactual_and_clean_STATE' and actual.get('direct_mode_readback_available') is False and actual.get('software_fallback_allowed') is False,'actual HES evidence not explicit')
        if verify_files:
            verify_ref(raw['mode_proof_ref']);proof=load(raw['mode_proof_ref']['path'])
            for name,after in (('without_hes',False),('after_hes',True)):
                verify_ref(proof[name]);validate_capability(load(proof[name]['path']),after)
...
def assess_blocks(rows,n):
    ...
    for state in p['states']:
        ...
        report={'state':state['id'],'required_processes':n*4,'reported_processes':len(group),'status':'rejected','reasons':[],'extension_eligible':False,'cost_points':[]}
        ...
        main_metric={'pair':'pair_span_ns','main':'independent_main_ns','convert':'independent_conversion_ns'}[state['role']]
        ...
        for metric in metric_names:
            ...
            # the actually admitted positive elapsed cost, not near-zero residuals.
            if metric==main_metric:all_stable=math.isfinite(spread) and spread<=.05
            report['cost_points'].append({'metric':metric,'median_ns':center,'block_medians_ns':block_medians,'max_block_relative_deviation':spread if math.isfinite(spread) else None,'is_primary_elapsed_cost':metric==main_metric})
        if not all_stable:report['reasons'].append('cross_block_elapsed_cost_variation_exceeds_5pct')
        report['extension_eligible']=n==5 and point_ok and not interval_ok and all_stable
        if interval_ok and all_stable:report['status']='qualified_exact_shape_observation'
    ...
    return {'schema':'r28-gpu-cost-evidence-evaluation/v1','blocks':n,'states':reports,'qualified_states':sum(r['status']=='qualified_exact_shape_observation' for r in reports),'required_states':5,
        'formal_success':all(r['status']=='qualified_exact_shape_observation' for r in reports),'simulator_coefficients_emitted':False,'held_out_M4':'unmeasured',
        'extension_eligible':n==5 and any(r['extension_eligible'] for r in reports) and all(r['status']=='qualified_exact_shape_observation' or r['extension_eligible'] for r in reports)}
...
def process_conflicts(records,own_pid=None):
    ...
    terms=('freeze_candidate.py','verify_frozen_preflight.py','run_wrapper_capture.py','run_target_capture_ex.py','host_submission_probe','gpu_operator_timing')
    for row in records:
        command=' '.join(row.get('cmdline') or []).replace('\\','/').lower();name=(row.get('name') or '').lower()
        if row['pid'] not in seen and (name=='gpu-operator-timing.exe' or (name.startswith('python') and any(t in command for t in terms))):bad.append(row)
    return bad

def assert_idle():
    bad=process_conflicts([p.info for p in psutil.process_iter(['pid','name','cmdline'])],os.getpid())
    need(not bad,'active or uninspectable simulation/native/freeze/probe processes: '+json.dumps(bad))

def runtime_prerequisites():
    assert_idle()
    r27_path=LOOP/'round_027/run_candidate.py';r27=b.load(r27_path,'r27_natural_terminal_guard')
    record,r27_ref=r27.barrier();need(record['terminal_count']==262 and record.get('failures_preserved') is True,'R27 has no complete natural terminal barrier')
    wp=LOOP/'round_026/mmvq_wrapper_launch_match';wrapper=b.load(wp/'run_wrapper_capture.py','r26_wrapper_qualification_guard')
    wrapper.verify_prepared();run=wp/'wrapper_capture_run.0001';finish=load(run/'finish.json')
    need(finish.get('status')=='fixed_shape_effective_launch_and_numerical_match' and finish.get('identity_unchanged') is True and finish.get('new_shim_numerically_qualified') is True,'new shim dynamic/numerical qualification not complete')
    for value in b.references(finish):verify_ref(value)
    raw=load(finish['launches_ref']['path'])
    source=load(wp/'source_manifest.json');wrapper.compare.compare(load(source['target_run']['path']),raw);wrapper.compare.verify_numerical_files(raw,verify_ref)
    assert_idle()
    return {'R27_barrier_ref':r27_ref,'R27_driver_ref':ref(r27_path),'wrapper_finish_ref':ref(run/'finish.json'),'wrapper_raw_ref':finish['launches_ref']}
...
def run_process(directory,kind,start,proof=None,state=None,mode=None):
    ...
    directory.mkdir(exist_ok=False);argv=[manifest['executable']['path'],'--protocol',str(P/'protocol.json'),'--kind',kind,'--output',str(directory)]
    if state is not None:argv+=['--state',state,'--mode',mode,'--proof',str(proof)]
    spec={'schema':'r28-gpu-timing-process-start/v1','created_utc':now(),'argv':argv,'kind':kind,'state':state,'mode':mode,'remaining_budget_seconds':remaining(start),'prepared_refs':start['prepared_refs']}
    write_new(directory/'start.json',spec)
    finish={'schema':'r28-gpu-timing-process-finish/v1','status':'rejected','returncode':None,'start_ref':ref(directory/'start.json'),'created_utc':None,'performance_parameter_admitted':False}
    ...
    try:
        ...
        with (directory/'stdout.log').open('xb') as stdout,(directory/'stderr.log').open('xb') as stderr:
            ...
            try:finish['returncode']=process.wait(timeout=min(90,remaining(start)))
            except subprocess.TimeoutExpired:
                process.kill();process.wait();finish['returncode']=process.returncode;raise RuntimeError('owned collector hit frozen process/deadline budget; retained timeout, no retry')
        need(finish['returncode']==0,'collector returned failure '+str(finish['returncode']))
        raw=load(directory/'raw.json')
        if kind.startswith('capability_'):validate_capability(raw,kind=='capability_after_hes')
        else:finish['summary']=validate_raw(raw,state,mode)
        finish['status']='valid'
    except Exception as e:finish['execution_error']=type(e).__name__+': '+str(e)
    ...
    except Exception as e:finish['identity_unchanged']=False;finish['identity_error']=type(e).__name__+': '+str(e);finish['status']='rejected'
    for key,name in (('raw_ref','raw.json'),('stdout_ref','stdout.log'),('stderr_ref','stderr.log')):finish[key]=ref(directory/name) if (directory/name).is_file() else None
...
def ensure_proof(directory,start):
    ...
    if destination.exists():
        proof=load(destination)
        for name,after in (('without_hes',False),('after_hes',True)):
            verify_ref(proof[name]);validate_capability(load(proof[name]['path']),after)
        return destination
    for name in ('without_hes','after_hes'):
        out=directory/('capability_'+name)
        if (out/'finish.json').is_file():
            result=load(out/'finish.json')
            for r in b.references(result):verify_ref(r)
        else:
            need(not out.exists(),'interrupted capability cannot be overwritten or silently retried')
            result=run_process(out,'capability_'+name,start)
        need(result['status']=='valid' and result.get('identity_unchanged') is True,'SDK26 counterfactual cannot establish HES; formal GPU timing blocked')
    proof={'schema':'r28-hes-counterfactual-proof/v1','created_utc':now(),'protocol_ref':ref(P/'protocol.json'),
        'without_hes':ref(directory/'capability_without_hes/raw.json'),'after_hes':ref(directory/'capability_after_hes/raw.json'),
        'direct_mode_readback_available':False,'mode_evidence':'documented HWTrace success plus controlled same-stage incompatible-API counterfactual and STATE review','software_fallback_allowed':False}
...
def campaign(directory,phase,commit=None):
    ...
    try:
        ...
        write_new(terminal,{'status':'completed','created_utc':now(),'evaluation_ref':ref(directory/f'evaluation_{n:02}.json'),'remaining_budget_seconds':remaining(start),'formal_success':report['formal_success']})
        return report
...
if __name__=='__main__':
    ...
    if args.phase=='check':prepared();print(json.dumps({'status':'compiled_and_host_tested','GPU_executed':False,'runtime_preconditions_checked':False,'M4_held_out':True}))
        ...
        result=campaign(args.run_dir,args.phase,args.reviewed_commit);print(json.dumps({'formal_success':result['formal_success'],'qualified_states':result['qualified_states'],'output':str(args.run_dir),'simulator_coefficients_emitted':False}))
