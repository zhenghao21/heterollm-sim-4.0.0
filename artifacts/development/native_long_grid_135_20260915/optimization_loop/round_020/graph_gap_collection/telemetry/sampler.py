...
class SamplerSession:
    ...
    def _worker(self):
        ...
        try:
            ...
            # One genuine post-stop read, by the owner thread, to bracket the
            # already-exited child's last formal interval. It is not a timed slot.
            start = self.timer.now()
            reading = reader.read_sm()
            end = self.timer.now()
            self.result['final_sample'] = {'sm_read_begin_qpc':start,'sm_read_end_qpc':end,'sm_mhz':reading,
                'phase':'after_owned_child_exit_stop','synthetic_sample':False}
