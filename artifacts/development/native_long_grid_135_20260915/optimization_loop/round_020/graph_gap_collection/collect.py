"""Finite R20 graph-gap A/B collector; root-frozen, serial, no timed termination.
...
PROBE=HERE.parent/'graph_gap_probe'
...
def plan(protocol,manifest):
    result=[];pilot=protocol['execution']['pilot'];require(pilot['config']==CONFIG['id'] and pilot['pairs_per_arm']==3 and pilot['native_processes']==12,'pilot domain')
    for pair in range(1,4):
        arms=['control','buffered'] if pair%2 else ['buffered','control']
        for arm in arms:
            modes=['direct','profile'] if (pair+(arm=='buffered'))%2 else ['profile','direct']
            for mode in modes+['export']:
                d=HERE/'runs'/arm/f'pair_{pair:02d}'/mode;raw=d/'raw.jsonl';pair_id=f"{pilot['id']}.{arm}.pair{pair}"
                app=[manifest['executable']['path'],'--run','--config',CONFIG['id'],'--arm',arm,'--pair-id',pair_id,'--output',str(raw)]
                if mode=='direct': argv=app
                elif mode=='profile':argv=[manifest['profiler']['path'],*protocol['profiler']['profile_options'],'--output='+str(d/'trace'),*app]
                else:argv=[manifest['profiler']['path'],'export','--type=sqlite','--force-overwrite=false','--output='+str(d/'trace.sqlite'),str(d.parent/'profile/trace.nsys-rep')]
                result.append({'ordinal':len(result),'arm':arm,'pair':pair,'pair_id':pair_id,'mode':mode,'config':CONFIG,'directory':str(d),'raw':str(raw),'app_argv':app,'argv':argv})
    return result
...
def prepare():
    ...
    for r in manifest['files']:verify(r)
    paths=[Path(__file__),HERE/'strict_raw.py',HERE/'native_trace.py',HERE/'quality_fixed.py',PROBE/'build_manifest.json',PROBE/'protocol.json']
    paths += list((HERE/'telemetry').glob('*.py'))
    smi=Path(shutil.which('nvidia-smi') or '')
    require(smi.is_file(),'nvidia-smi executable required')
    record={'schema':'r20-graph-gap-collection-freeze/v1','created_utc':utc(),'manifest':ref(PROBE/'build_manifest.json'),'protocol':ref(PROBE/'protocol.json'),'collector_inputs':[ref(p) for p in paths], 'native_and_tool_refs':manifest['files'],'smi':ref(smi),'plan':plan(protocol,manifest),'expected_native_processes':12,'expected_export_processes':6,'coefficients_allowed':False,'resume_policy':'unresolved or failed series is retained, no success-seeking retries','clock':{'target_mhz':2400,'tolerance_mhz':30,'period_ms':5,'max_bracket_ms':25},'timing_policy':{'p90_p10_max':1.5,'process_median_deviation_max':0.05,'profile_direct_wall_relative_max':0.20},'first_pair_review_required':True}
...
def verified_freeze(expected):
    ...
    require(len(f['plan'])==18 and sum(x['mode']!='export' for x in f['plan'])==12,'full plan denominator')
    return f
...
def stage_run(stage,freeze_sha,manifest,protocol,clock_ref):
    ...
    try:
        ...
        if session:
            time.sleep(.01) # after process exit, collect a right-hand clock bracket; outside every timed graph.
            session.stop()
            while session.thread.is_alive():
                try:session.join(1)
                except TimeoutError:continue
            data=session.result;session.close()
    ...
    try:
        if stage['mode']!='export':
            rawdoc=audit(stage['raw'],stage,manifest,protocol,proc.pid if stage['mode']=='direct' else None)
...
def summarize_pairs(freeze_sha,partial=False):
    ...
    f=verified_freeze(freeze_sha);manifest=load(f['manifest']['path']);protocol=load(f['protocol']['path']);groups=[]
    for arm in ('control','buffered'):
        pairs=[]
        for pair in range(1,4):
            root=HERE/'runs'/arm/f'pair_{pair:02d}'
            if not all((root/m/'complete.json').exists() for m in ('direct','profile','export')):continue
...
def run(expected):
    ...
    result={'freeze_sha256':expected,'started_utc':utc(),'status':'started','completed_stages':0};lock_attempted=False
    try:
        ...
        for stage in f['plan']:
            ...
            if result['completed_stages']==6:
                summary=summarize_pairs(expected,True);write_new(HERE/'first_pair_summary.json',summary)
                write_new(HERE/'first_pair_ready.json',{'freeze_sha256':expected,'clock_ref':clock_ref,'controller_pid':os.getpid(),'review_required':True})
                deadline=time.monotonic()+300
                while not (HERE/'continue.json').exists():
                    require(time.monotonic()<deadline,'first pair root review window expired; no children terminated');time.sleep(1)
                consent=load(HERE/'continue.json');require(consent.get('freeze_sha256')==expected and consent.get('first_pair_reviewed') is True,'continuation identity mismatch')
            print(json.dumps({'completed_stages':result['completed_stages'],'expected_stages':18}),flush=True)
        write_new(HERE/'summary.json',summarize_pairs(expected));result['status']='completed'
    except BaseException as exc:
        result.update(status='failed_or_incomplete',error=type(exc).__name__+': '+str(exc))
    finally:
        if lock_attempted:result['reset']=smi(['-rgc'],'reset')
        result['finished_utc']=utc();result['expected_stages']=18
        result['stage_status']=[{'ordinal':x['ordinal'],'arm':x['arm'],'pair':x['pair'],'mode':x['mode'],'process_status':load(Path(x['directory'])/'complete.json').get('status') if (Path(x['directory'])/'complete.json').exists() else 'not_run','validation_status':load(Path(x['directory'])/'validation.json').get('status') if (Path(x['directory'])/'validation.json').exists() else 'not_validated'} for x in f['plan']]
