...
constexpr unsigned WARMUP_CALLS = 16, STEADY_REPEATS = 64, PROCESS_COUNT = 6;
...
struct CpuObservation {
    ...
    std::string json() const {
        return "{\"group\":0,\"logical_processor\":" + std::to_string(logical) + ",\"thread_affinity_mask\":" + std::to_string(affinity)
...
static std::string run_case(NativeSampler& native, int n, bool random_pattern, unsigned cpu, const std::string & reset_policy) {
    ...
    std::vector<llama_token_data> baseline(n), work(n);
    for (int i=0;i<n;++i) baseline[i] = {i, logits[i], 0.0f};
    // R20 A/B: reset is deliberately outside the QPC window. The work vector
    // is allocated once per case before either arm and both arms target this
    // same storage. This records reset policy, not a new timed candidate stage.
    ...
    auto * const work_storage = work.data(); const auto work_capacity = work.capacity();
    auto reset = [&] {
        if (reset_policy == "memcpy_baseline") {
            std::memcpy(work.data(), baseline.data(), size_t(n)*sizeof(llama_token_data));
            view = { work.data(), size_t(n), -1, false };
            check_candidate(logits, work, view);
        } else if (reset_policy == "source_candidate_loop") {
            candidate_loop(logits.data(), n, work, view);
            check_candidate(logits, work, view);
        } else {
            throw std::runtime_error("unknown reset policy");
        }
        require(work.data() == work_storage && work.capacity() == work_capacity, "reset reallocated work buffer");
        _ReadWriteBarrier();
    };
    ...
    return "{\"vocabulary_size\":"+std::to_string(n)+",\"pattern\":"+quote(random_pattern?"deterministic_random_permutation":"monotone_ascending")
        +",\"split\":"+quote("quality_diagnosis")+",\"input_seed\":"+std::to_string(INPUT_SEED)
        ...
        +",\"candidate_record_bytes\":12,\"top_k\":1,\"native_sampler_reused_stateless\":true,\"native_topk_reset_before_every_call\":true,\"reset_policy\":"+quote(reset_policy)+",\"reset_outside_clock_window\":true"
        +",\"first_use_not_pooled_with_steady\":true,\"stages\":["+topk.json()+"]}";
}
...
int wmain(int argc, wchar_t** argv) {
    try {
        if (argc == 2 && std::wstring(argv[1]) == L"--observation-schema-fixture") {
            const CpuObservation sample{0, 4300, 4300, 4300, 1};
            std::cout << "{\"schema\":\"cpu-observation-schema-fixture/v1\",\"synthetic\":true,\"DLL_loaded\":false,\"GPU_calls\":false,\"cpu_observation\":" << sample.json() << "}\n";
            return 0;
        }
        fs::path dll, runtime_dir, cuda_dir, output; int process_index=-1, cpu=-1; std::string reset_policy; bool approved=false, identity_only=false;
        for(int i=1;i<argc;++i) {
            ...
            if(option==L"--measure-root-approved" || option==L"--identity-root-approved") approved=true;
                ...
                else if(option==L"--cpu-index") cpu=std::stoi(value); else if(option==L"--reset-policy") reset_policy=utf8(value); else throw std::runtime_error("unknown option");
            }
        }
        if (identity_only) {
            ...
            file.write("{\"schema\":\"cpu-reset-probe-actual-cpu-identity/v1\",\"status\":\"complete\",\"GPU_context_created\":false,\"model_loaded\":false,\"timing_values_produced\":false,\"actual_cpu_identity\":" + identity.json() + "}");
            return 0;
        }
        require(dll.is_absolute(), "relative native DLL path is forbidden");
        require(approved && process_index>=0 && process_index<6 && cpu>=0 && cpu<64 && !dll.empty() && runtime_dir.is_absolute() && cuda_dir.is_absolute() && (reset_policy == "memcpy_baseline" || reset_policy == "source_candidate_loop") && !output.empty(), "explicit root measurement approval, process-index 0..5, cpu-index, native-dll, frozen runtime/cuda directories, reset policy and output required");
        ...
        try {
            ...
            observer_ticks+=']';
            metadata="\"schema\":\"cpu-reset-independent-probe/v1\",\"GPU_context_created\":false,\"GPU_context_created_by_probe_calls\":false,\"no_probe_gpu_api_calls\":true,\"dependency_dlls_loaded\":true,\"dependency_loading_is_not_GPU_measured\":true,\"no_dllmain_context_claim\":true,\"model_loaded\":false,\"full_sampler_chain_measured\":false"
            ...
            for(int offset=0;offset<6;++offset) {
                ...
                std::string item=run_case(native,VOCABS[case_id/2],bool(case_id%2),unsigned(cpu),reset_policy);
