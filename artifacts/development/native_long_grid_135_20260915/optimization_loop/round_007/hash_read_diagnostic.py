...
# Keep ctime as evidence and compare it within each API; Windows path stat
# and handle fstat can expose different ctime semantics on the same file.
CROSS_API_IDENTITY_KEYS = ("st_dev", "st_ino", "st_size", "st_mtime_ns", "st_mode", "st_file_attributes")


def same_cross_api_identity(left, right):
    return all(left[key] == right[key] for key in CROSS_API_IDENTITY_KEYS)
...
def collect_pass(target, expected_sha, output_dir, ordinal):
    ...
    try:
        ...
        record["cross_api_identity_keys"] = list(CROSS_API_IDENTITY_KEYS)
        record["path_and_handle_identity_match"] = (
            same_cross_api_identity(record["path_before"], record["handle_before"])
            and same_cross_api_identity(record["path_after"], record["handle_after"])
        )
