...
ap=argparse.ArgumentParser();ap.add_argument('variant',choices=['current','sampling','pure']);a=ap.parse_args()
...
result=freeze_selection(Path(f['selection_ref']['path']),P/a.variant,data_root=Path(f['data_root']),
 ...
 tensor_storage_f32_hidden=True,gpu_invocation_contract_path=path,gpu_mmq_source_costs=a.variant!='pure',gpu_conversion_cta_costs=False,
 sampling_contract_path=(P/'sampling_parity/static_sampling_contract.json') if a.variant=='sampling' else None)
