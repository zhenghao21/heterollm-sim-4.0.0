...
static std::string modules_json() {
    ...
    if(Module32FirstW(snapshot,&entry))do {
        ...
        if(lowered.find("ggml")!=std::string::npos||lowered.find("cuda")!=std::string::npos||lowered.find("cublas")!=std::string::npos||lowered=="operator-surface-probe.exe") {
...
struct Options {
    std::string device="cuda",format="Q5_0",output; bool control=false;
    int64_t m=0,n=0,k=0;int batch=1,threads=1,cuda_index=0,warmup=5,repeats=30,samples=4096;
    uint32_t seed=20260914;double atol=0.05,rtol=0.03;bool run=false,nvtx=true;int evict_mib=128;
};
...
static Options parse(int argc,char **argv) {
    Options o;for(int i=1;i<argc;++i) {
        ...
        if(a=="--help"){std::cout<<"Use invoke.ps1 -Config <frozen26 id> for identity-only checking. GPU run requires -Run -RootReviewed -IdleWindowConfirmed. One graph per call, cold-cache sweep, NVTX in both direct/profile, event/control pairing. Binary check-only accesses GPU; do not use for host checks.\n";std::exit(0);}
    ...
    const bool train=o.k==896&&(o.m==1||o.m==4||o.m==64)&&(o.n==128||o.n==896||o.n==4864);
    const bool validation=o.k==896&&(o.m==2||o.m==32)&&o.n==1792;
    const bool aligned=o.k==1024&&(o.m==4||o.m==64)&&o.n==896;
    if(o.device!="cuda"||(o.format!="Q5_0"&&o.format!="Q8_0")||!(train||validation||aligned))throw std::runtime_error("configuration outside frozen 26-config protocol");
    if(o.batch!=1||o.threads!=1||o.warmup!=5||o.repeats!=30||o.samples!=4096||o.seed!=20260914||o.atol!=0.05||o.rtol!=0.03||o.evict_mib!=128||!o.nvtx||o.cuda_index!=0)throw std::runtime_error("protocol override forbidden");
    return o;
}
...
// Explicit absolute loads include ggml.dll even when /OPT:REF removes every
// import of that library. Keep our references until Resources and CUDA events
// have been destroyed, and retain the existing repeated identity checks.
struct FrozenNativeModuleApi {
    using Handle = HMODULE;
    void require_absolute_path(const char *path) const {
        std::string text(path);
        if (text.size() < 3 || !std::isalpha(static_cast<unsigned char>(text[0]))
                || text[1] != ':' || (text[2] != '\\' && text[2] != '/'))
            throw std::runtime_error(std::string("frozen DLL path must be drive-absolute: ") + path);
    }
    HMODULE load_absolute(const char *path) const {
        HMODULE handle = LoadLibraryExA(path, nullptr,
            LOAD_LIBRARY_SEARCH_DLL_LOAD_DIR | LOAD_LIBRARY_SEARCH_DEFAULT_DIRS);
        if (!handle) throw std::runtime_error(std::string("failed to preload frozen DLL: ") + path
            + " (Win32 error " + std::to_string(GetLastError()) + ")");
        return handle;
    }
    void verify_handle(HMODULE handle, const char *expected_path, const char *expected_hash) const {
        char actual[32768]; DWORD n = GetModuleFileNameA(handle, actual, sizeof(actual));
        if (!n || n >= sizeof(actual) || _stricmp(actual, expected_path) != 0
                || file_hash(actual) != expected_hash)
            throw std::runtime_error(std::string("preloaded DLL identity mismatch: ") + expected_path);
    }
...
static Check validate(ggml_tensor *out,const std::vector<uint8_t>&packed,const std::vector<float>&input,const Options&o,ggml_type type) {
    ...
    for(int j=0;j<c.count;++j) {
        ...
        double path_ref=source_path_reference(packed.data()+ni*ggml_row_size(type,o.k),input.data()+mi*size_t(o.k),int(o.k),type,o.m<=4);
        double path_error=std::abs(double(actual[i])-path_ref);
        bool okay=std::isfinite(actual[i])&&std::isfinite(path_ref)&&path_error<=1e-4+1e-5*std::abs(path_ref);
...
static int run(const Options &o,std::string &json) {
    verify_files();
    FrozenNativeModuleApi module_api;
    PinnedFrozenModules<FrozenNativeModuleApi> pinned_modules(module_api);
    pinned_modules.preload(frozen_modules);
    ...
    auto compute=[&](const std::string &phase,int index){
        Run v;v.phase=phase;v.index=index;
        v.pre_sync_start=qpc();ggml_backend_synchronize(r.backend);v.pre_sync_end=qpc();cuda_ok(cudaGetLastError(),"before cache eviction");
        if(!evict_graph||evict_bytes<size_t(l2_bytes)*4)throw std::runtime_error("cold-cache sweep contract not satisfied");
        v.evict_start=qpc();v.evict_status=int(ggml_backend_graph_compute_async(r.backend,evict_graph));v.evict_submit_end=qpc();
        if(v.evict_status!=GGML_STATUS_SUCCESS)throw std::runtime_error("eviction graph failed");
        ggml_backend_synchronize(r.backend);v.evict_end=qpc();cuda_ok(cudaGetLastError(),"cache eviction synchronize");
        v.label="operator_surface/v1|phase="+phase+"|index="+std::to_string(index)+"|op=MUL_MAT|M="+std::to_string(o.m)+"|N="+std::to_string(o.n)+"|K="+std::to_string(o.k)+"|quant="+o.format+"|input=F32|output=F32|layout=contiguous2d|expected_path="+(o.m<=4?"MMVQ_Q8_1_HALF":"MMQ_Q8_1_D4_F32");
        v.nvtx_push_start=qpc();nvtxRangePushA(v.label.c_str());v.nvtx_push_end=qpc();
        v.start=qpc();
        ...
        v.submit_start=qpc();v.status=int(ggml_backend_graph_compute_async(r.backend,g));v.submit_end=qpc();++v.graph_count;
        v.submit_cuda_status=int(cudaGetLastError());cuda_ok(cudaError_t(v.submit_cuda_status),"async submission");
        if(v.status!=GGML_STATUS_SUCCESS)throw std::runtime_error("async graph status failure");
        if(!o.control){
        ...
        v.nvtx_pop_start=qpc();nvtxRangePop();v.nvtx_pop_end=qpc();
        if(!(v.evict_start<=v.evict_submit_end&&v.evict_submit_end<=v.evict_end&&v.evict_end<=v.nvtx_push_start&&v.nvtx_push_start<=v.nvtx_push_end&&v.nvtx_push_end<=v.start&&v.start<=v.submit_start&&v.submit_start<=v.submit_end&&v.submit_end<=v.wait_start&&v.wait_start<=v.wait_end&&v.wait_end<=v.nvtx_pop_start&&v.nvtx_pop_start<=v.nvtx_pop_end))throw std::runtime_error("QPC ordering failed");
        ...
        v.validation_start=qpc();auto c=validate(out,packed,input,o,type);v.validation_end=qpc();v.correct=c.passed;v.correctness=check_json(c);runs.push_back(v);return c.passed;
    };
    bool okay=compute("first_call",0);Check first_check,final_check;
    first_check=validate(out,packed,input,o,type);okay=first_check.passed&&okay;
    for(int i=0;i<o.warmup;++i){bool pass=compute("warmup",i);okay=pass&&okay;}
    for(int i=0;i<o.repeats;++i){bool pass=compute("formal",i);okay=pass&&okay;}
    final_check=validate(out,packed,input,o,type);okay=final_check.passed&&okay;
    ggml_backend_synchronize(r.backend);cuda_ok(cudaGetLastError(),"final synchronize");events.close();verify_loaded_modules();
    auto modules_after=modules_json();okay=okay&&(modules_before==modules_after);std::ostringstream all;all<<std::setprecision(17)<<'{'+header.str()<<",\"status\":"<<quote(okay?"measured":"failed_compute_or_correctness")<<",\"graph_compute_calls\":"<<(runs.size()*size_t(o.batch))<<",\"quantization\":{\"method\":\"ggml_quantize_chunk\",\"bytes\":"<<quantized<<",\"importance_matrix\":"<<quote(importance?"synthetic_ones_declared_no_LLM_data":"not_required")<<",\"packed_weight_sha256\":"<<quote(sha256_bytes(packed.data(),packed.size()))<<",\"input_sha256\":"<<quote(sha256_bytes(input.data(),input.size()*sizeof(float)))<<"}";
    all<<",\"correctness_contract\":{\"reference\":\"ggml_to_float_quantized_weight_double_dot_F32_input\",\"sample_policy\":\"uniform_output_flat_indices\",\"absolute_tolerance\":"<<o.atol<<",\"relative_tolerance\":"<<o.rtol<<",\"path_absolute_tolerance\":0.0001,\"path_relative_tolerance\":0.00001,\"reference_mode\":\"dual_math_and_source_path\",\"source_runtime_equivalence_proven\":false,\"criterion\":\"Both formats source-path <=1e-4+1e-5*abs(path); preserve mathematical errors; expected dispatch must be confirmed by trace\"},\"first_call_correctness\":"<<check_json(first_check)<<",\"final_correctness\":"<<check_json(final_check)<<",\"runs\":[";
    for(size_t i=0;i<runs.size();++i){auto&v=runs[i];if(i)all<<',';all<<std::setprecision(17);
    all<<"{\"phase\":"<<quote(v.phase)<<",\"index\":"<<v.index<<",\"qpc_start\":"<<v.start<<",\"qpc_end\":"<<v.end<<",\"graph_computations\":"<<v.graph_count<<",\"host_wall_ns\":"<<double(v.end-v.start)*1e9/double(qpf())<<",\"host_per_graph_ns\":"<<double(v.end-v.start)*1e9/double(qpf())/o.batch<<",\"qpc_record_begin_start\":"<<v.record_begin_start<<",\"qpc_record_begin_end\":"<<v.record_begin_end<<",\"qpc_submit_start\":"<<v.submit_start<<",\"qpc_submit_end\":"<<v.submit_end<<",\"qpc_record_end_start\":"<<v.record_end_start<<",\"qpc_record_end_end\":"<<v.record_end_end<<",\"qpc_wait_start\":"<<v.wait_start<<",\"qpc_wait_end\":"<<v.wait_end<<",\"ggml_status\":"<<v.status<<",\"cuda_begin_record_status\":"<<v.begin_record_status<<",\"cuda_end_record_status\":"<<v.end_record_status<<",\"cuda_submit_status\":"<<v.submit_cuda_status<<",\"cuda_query_before_wait\":"<<v.query_before<<",\"cuda_wait_status\":"<<v.wait_status<<",\"cuda_query_after_wait\":"<<v.query_after<<",\"cuda_elapsed_status\":"<<v.elapsed_status<<",\"event_envelope_ms\":";if(o.control)all<<"null";else all<<std::setprecision(9)<<v.event_ms;all<<std::setprecision(17)<<",\"nvtx_label\":"<<quote(v.label)<<",\"qpc_pre_sync_start\":"<<v.pre_sync_start<<",\"qpc_pre_sync_end\":"<<v.pre_sync_end<<",\"qpc_evict_start\":"<<v.evict_start<<",\"qpc_evict_submit_end\":"<<v.evict_submit_end<<",\"qpc_evict_end\":"<<v.evict_end<<",\"qpc_nvtx_push_start\":"<<v.nvtx_push_start<<",\"qpc_nvtx_push_end\":"<<v.nvtx_push_end<<",\"qpc_nvtx_pop_start\":"<<v.nvtx_pop_start<<",\"qpc_nvtx_pop_end\":"<<v.nvtx_pop_end<<",\"qpc_validation_start\":"<<v.validation_start<<",\"qpc_validation_end\":"<<v.validation_end<<",\"eviction_status\":"<<v.evict_status<<",\"correctness\":"<<v.correctness<<'}';}
...
int main(int argc,char**argv) {
    ...
    try{options=parse(argc,argv);status=run(options,result);}catch(const std::exception&e){result="{\"schema\":\"single-operator-surface-probe/v2\",\"status\":\"error\",\"error\":"+quote(e.what())+"}";}
    if(options.output.empty())std::cout<<result<<'\n';else std::cout<<"{\"status_code\":"<<status<<",\"raw_output\":"<<quote(options.output)<<"}\n";
