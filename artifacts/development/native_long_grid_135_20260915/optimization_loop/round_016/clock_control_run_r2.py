...
def main(argv=None):
    ...
    if any(P.glob(PREFIX+'*')):raise RuntimeError('clock-control-r3 output prefix already exists; never overwrite or start another session')
    ...
    try:
        ...
        if protocol['quality_policy']['config_count']!=26 or len(protocol['execution_plan'])!=234:raise RuntimeError('unapproved matrix scope/order')
        if (C/'runs').exists() and any((C/'runs').iterdir()):raise RuntimeError('new controller requires fresh collection_r3; old session cannot be reused')
        ...
        if len(receipts)!=234:raise RuntimeError('matrix final denominator mismatch')
