...
import sqlite3
import statistics
from pilot import HERE, BASE, NSYS, ref, utc, verify_build, verify_freeze, write_json
...
ATTRIBUTION_SOURCE = NSYS.parent / 'reports' / 'nvtx_gpu_proj_trace.py'
...
def analyze():
    ...
    if any((HERE / name).exists() for name in ('result_summary.json', 'mapped_calls.json', 'extraction_before_verification.json')):
        raise RuntimeError('extraction already attempted; preserve evidence and use a new extractor version')
    write_json(HERE / 'extraction_before_verification.json', before, exclusive=True)
    ...
    write_json(HERE / 'observed_sqlite_schema.json', describe_schema(connection), exclusive=True)
    ...
    summary = {'schema': 'synthetic-gemm-trace-pilot-result/v1', 'created_utc': utc(),
        ...
        'limits': (['Captured diagnostics report unsupported driver or collection fallback.'] if compatibility else []) + [
    ...
    summary['freeze_after_extraction'] = verify_freeze()
    write_json(HERE / 'result_summary.json', summary, exclusive=True)
    connection.close()
