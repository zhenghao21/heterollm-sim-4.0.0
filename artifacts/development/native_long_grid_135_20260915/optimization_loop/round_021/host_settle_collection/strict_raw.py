"""R21 buffered host-settle raw evidence contract. No native calls, no cost fitting."""
from pathlib import Path
import json,hashlib,re,math
from native_trace import distribution,expected_calls,ref
from contract import domain,stage_identity
...
def audit(path, stage, manifest, protocol, process_pid=None, extra_runtime_refs=()):
    domain(protocol,manifest);stage_identity(stage,manifest)
    ...
    expected_ms=stage['settle_ms'];started=one('buffered_started');settle=one('settle_metadata')
    require(started.get('arm')=='buffered' and started.get('config')==config['id'] and started.get('condition_settle_ms')==expected_ms and started.get('first_timed_ordinal')==0 and rows.index(started)<rows.index(settle), 'durable buffered start identity')
    require(settle.get('arm')=='buffered' and type(settle.get('condition_settle_ms')) is int and settle['condition_settle_ms']==expected_ms and settle.get('qpc_frequency')==h['qpc_frequency'] and settle.get('graph_status_pass') is True and settle.get('estimator_time_used') is False, 'settle identity/status/condition')
    require(a.get('write_cadence')==f'buffered_settle_{expected_ms}ms', 'settle write cadence condition')
    begin,end,iterations=settle.get('qpc_begin'),settle.get('qpc_end'),settle.get('iterations')
    require(type(begin) is int and type(end) is int and calls[0]['qpc_outer_pop_end']<=begin<=end<=calls[1]['qpc_outer_push_start'], 'settle QPC placement outside recorded calls')
    require(type(iterations) is int and (iterations==0 if expected_ms==0 else iterations>0) and (expected_ms==0 or (end-begin)*1000>=expected_ms*h['qpc_frequency']), 'settle iteration/work duration')
    # The real r2 emitter flushes first/warmup/formal records after settle metadata.
    # Temporal placement is proved by QPC above, never by JSONL call line ordering.
    ...
    require(f.get('arm')==arm and f.get('math_pass')==math_pass and f.get('graph_status_pass') is True and f.get('status')==('complete' if math_pass else 'quality_failed'),'footer status closure')
    side=load(str(path)+'.receipt.json');actual=ref(path)
    require(side.get('schema')=='graph-gap-run-receipt/v1' and side.get('status')==f['status'] and side.get('calibration_ready') is False and side.get('raw_sha256')==actual['sha256'] and side.get('raw_bytes')==actual['bytes'] and side.get('raw_path')==str(path) and side.get('protocol_sha256')==manifest['protocol']['sha256'] and side.get('arm')==arm and side.get('pair_id')==pair and side.get('config')==config['id'],'closed raw sidecar identity')
    ...
    return {'raw_ref':actual,'sidecar_ref':ref(str(path)+'.receipt.json'),'header':h,'setup':s,'footer':f,'calls':calls,'numeric_blocks':blocks,'math_pass':math_pass,'per_call_final_validated':False,'settle_metadata':settle,'condition':stage.get('condition'),'settle_ms':stage.get('settle_ms'),'host_formal':host,'formal_clock_intervals':[{'index':x['index'],'qpc_start':x['qpc_outer_push_start'],'qpc_end':x['qpc_outer_pop_end']} for x in formal]}