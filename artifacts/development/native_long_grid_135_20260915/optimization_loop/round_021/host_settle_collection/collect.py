"""Finite R21 host-settle buffered A/B collector; root-frozen, serial, no timed termination.
...
PROBE=HERE.parent/'host_settle_probe_r2'
from contract import CONFIG, CONDITIONS, domain
...
def plan(protocol,manifest):
    result=[]; domain(protocol,manifest)
    for pair in range(1,4):
        conditions=['short','settled'] if pair%2 else ['settled','short']
        for condition in conditions:
            settle=0 if condition=='short' else 1000; modes=['direct','profile'] if (pair+(condition=='settled'))%2 else ['profile','direct']
            for mode in modes+['export']:
                d=HERE/'runs'/condition/f'pair_{pair:02d}'/mode;raw=d/'raw.jsonl';pair_id=f"r21_host_settle.{condition}.pair{pair}"
                app=[manifest['executable']['path'],'--run','--config',CONFIG['id'],'--arm','buffered','--settle-ms',str(settle),'--pair-id',pair_id,'--output',str(raw)]
                ...
                result.append({'ordinal':len(result),'condition':condition,'settle_ms':settle,'arm':'buffered','pair':pair,'pair_id':pair_id,'mode':mode,'config':CONFIG,'directory':str(d),'raw':str(raw),'app_argv':app,'argv':argv})
    return result
def prepare():
...
def summarize_pairs(freeze_sha,partial=False):
    ...
    for condition in CONDITIONS:
        arm='buffered'; condition_id=condition['id']
        pairs=[]
        for pair in range(1,4):
            root=HERE/'runs'/condition_id/f'pair_{pair:02d}'
            if not all((root/m/'validation.json').exists() and load(root/m/'validation.json').get('status')=='structurally_validated' for m in ('direct','profile','export')):continue
