"""R21 immutable saved-evidence report: anchors three-way, or physical full131.
Only standard-library imports; never imports simulator/evaluator or runs predictions.
Usage: python summarize_ablation.py anchors|full [--directory ROUND_DIR] [--heatmap]
All validation precedes exclusive creation of phase-specific JSON/Markdown/SVG.
"""
...
def load_bundle(directory,variant,full=False):
    ...
    require(actual=={ident+'.prediction.json' for ident in planned},'saved terminal prediction set differs from planned '+str(len(planned))+': '+variant)
    predictions={};refs={}
...
def check_controls(directory):
    controls,control_ref=read_json(directory/'evaluation_controls.json');protocol,protocol_ref=read_json(directory/'evaluation_protocol.json')
    require(controls.get('schema')=='ablation-controls/v1' and controls.get('same_source_closure') is True,'control manifest identity')
    refs=[verify_reference(x) for x in controls.get('files',[])];paths={x['path'] for x in refs}
    required=[Path(__file__).resolve(),directory/'evaluate_candidate.py',directory/'evaluation_protocol.json',directory/'nonflash_kv_view_source_contract.json',*(directory/v/'freeze.json' for v in VARIANTS)]
    require(len(paths)==len(refs) and all(str(q.resolve()) in paths for q in required),'unfrozen evaluator/summarizer/protocol/variant dependency')
    ...
    return protocol,{'controls':control_ref,'protocol':protocol_ref,'files':refs}
    ...
    for name in ('runtime_build_audit','recurrent_batching','cpu_iq_panel_reuse','slot_order','host_offload_source','tensor_storage','gpu_invocation','sampling','nonflash_kv_view'):
        ...
        if binding:
            for item in binding.get('evidence_refs',[])+([binding['audit_ref']] if 'audit_ref' in binding else []):
                r=normalized_ref(item);old=refs.setdefault(r['path'],r);require(old==r,'conflicting evidence reference')
    return list(refs.values())


def load_score(bundle,name,expected_ids):
    doc,sr=read_json(bundle['directory']/name);require(doc.get('schema')=='stable-native-simulation-errors/v1' and doc.get('selected_denominator')==DENOMINATOR,'score schema/131 denominator')
...
def assess(directory,phase):
    ...
    bundles={v:load_bundle(directory/v,v,full=phase=='full' and v=='physical') for v in VARIANTS}
    ...
    for b in bundles.values():
        for r in evidence_closure(b['freeze']):
            expected=closure.setdefault(r['path'],r);require(expected==r,'cross-variant evidence identity conflict')
    for r in closure.values():verify_reference(r)
    # All requested saved predictions were loaded and hashed before reading scores.
    anchor_scores={v:load_score(b,'errors.0001.json',ids) for v,b in bundles.items()}
    ...
    if phase=='full':
        ...
        full=load_score(b,'errors.0002.json',b['planned'])
