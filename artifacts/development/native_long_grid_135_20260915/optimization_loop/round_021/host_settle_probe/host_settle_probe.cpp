...
static long long settle_qpf() { LARGE_INTEGER v{}; if (!QueryPerformanceFrequency(&v) || v.QuadPart <= 0) throw std::runtime_error("QPC frequency unavailable"); return v.QuadPart; }

template <class Emit, class Flush>
ArmResult run_settle_buffered_arm(Harness &harness, const ArmContext &context, int settle_ms, Emit emit, Flush durable_flush) {
    if (context.arm != "buffered" || (settle_ms != 0 && settle_ms != 1000)) throw std::runtime_error("R21 buffered settle condition guard");
    std::array<CallSample, kTotalCalls> calls{};
    std::array<ValidationBlock, 3> blocks{};
    std::array<bool, 3> ready{};
    int completed = 0;
    bool settle_status = true;
    auto flush_records = [&] {
        emit_arm_metadata(context, settle_ms == 0 ? "buffered_settle_0ms" : "buffered_settle_1000ms", emit);
        for (int i=0;i<completed;++i) emit(call_json(context,calls[i]));
        for (std::size_t i=0;i<blocks.size();++i) if(ready[i]) emit(validation_json(context,blocks[i]));
        durable_flush();
    };
    try {
        emit("{\"record\":\"buffered_started\",\"arm\":"+quote(context.arm)+",\"config\":"+quote(context.config)+",\"condition_settle_ms\":"+std::to_string(settle_ms)+",\"first_timed_ordinal\":0}");
        durable_flush();
        collect_phase(harness,context,0,kFirstCalls,calls,completed);
        blocks[0]=check_all_stages(harness,"first",0);ready[0]=true;
        char label[192]{};
        std::snprintf(label,sizeof(label),"graph_submit/%s/settle/%dms",context.config.c_str(),settle_ms);
        Range settle(label);
        const long long begin=qpc(), frequency=settle_qpf();
        const long long deadline=begin+frequency*settle_ms/1000;
        int iterations=0;
        while(settle_ms>0 && qpc()<deadline) {
            const int status=int(ggml_backend_graph_compute_async(harness.backend,harness.graph));
            ggml_backend_synchronize(harness.backend);
            ++iterations;
            if(status!=GGML_STATUS_SUCCESS){settle_status=false;break;}
        }
        const long long end=qpc();settle.close();
        emit("{\"record\":\"settle_metadata\",\"arm\":"+quote(context.arm)+",\"condition_settle_ms\":"+std::to_string(settle_ms)+",\"qpc_begin\":"+std::to_string(begin)+",\"qpc_end\":"+std::to_string(end)+",\"qpc_frequency\":"+std::to_string(frequency)+",\"iterations\":"+std::to_string(iterations)+",\"graph_status_pass\":"+(settle_status?"true":"false")+",\"estimator_time_used\":false}");
        durable_flush();
        if(!settle_status)throw std::runtime_error("settle graph status failed");
        collect_phase(harness,context,kFirstCalls,kWarmupCalls,calls,completed);
        blocks[1]=check_all_stages(harness,"post_warmup",kFirstCalls+kWarmupCalls-1);ready[1]=true;
        collect_phase(harness,context,kFirstCalls+kWarmupCalls,kFormalCalls,calls,completed);
        blocks[2]=check_all_stages(harness,"post_formal",kTotalCalls-1);ready[2]=true;
    } catch(...) {flush_records();throw;}
    ArmResult result;
    for(const auto &sample:calls){++result.graph_calls;result.graph_status_pass=result.graph_status_pass && sample.graph_status==GGML_STATUS_SUCCESS;}
    result.graph_status_pass=result.graph_status_pass && settle_status;
    for(const auto &block:blocks)include_block(result,block);
    flush_records();return result;
}
} // namespace graph_gap_probe