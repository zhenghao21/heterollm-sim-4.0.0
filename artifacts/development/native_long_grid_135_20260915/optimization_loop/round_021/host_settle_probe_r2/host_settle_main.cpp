...
static Options parse_options(int argc, char **argv) {
    ...
    for (int index = 1; index < argc; ++index) {
        ...
        if (argument == "--settle-ms") { if (value != "0" && value != "1000") throw std::runtime_error("settle-ms must be exactly 0 or 1000"); options.settle_ms = std::stoi(value); }
    ...
    if (options.run && (options.config.empty() || options.arm.empty() || options.pair_id.empty() || options.output.empty())) {
        throw std::runtime_error("--run requires --config, --arm buffered, --settle-ms 0|1000, --pair-id and new --output");
    }
    if (options.run && (options.arm != "buffered" || (options.settle_ms != 0 && options.settle_ms != 1000))) throw std::runtime_error("R21 requires --arm buffered and --settle-ms 0|1000");
    return options;
}
...
static RunSummary run_probe(const Options &options, Jsonl &raw, int argc, char **argv) {
    ...
    RunSummary summary;
    summary.arm = graph_gap_probe::run_settle_buffered_arm(harness, context, options.settle_ms, emit, [&raw] { raw.durable(); });
    verify_files();
    verify_loaded_modules();
