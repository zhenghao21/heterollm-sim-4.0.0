...
try {
 ...
 $config=$protocol.execution.pilot.config
 ...
 $condition=if($SettleMs -eq 0){'short'}else{'settled'}
 if(@($protocol.execution.pilot.conditions | Where-Object {$_.settle_ms -eq $SettleMs -and $_.id -eq $condition}).Count -ne 1){throw 'Protocol settle condition mismatch'}
 $pairId="$($protocol.execution.pilot.id).$condition.pair$Pair"
