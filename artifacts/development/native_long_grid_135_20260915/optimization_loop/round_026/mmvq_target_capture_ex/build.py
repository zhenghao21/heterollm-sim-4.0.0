...
SOURCES = ('launch_decode.h', 'launch_recorder.cpp', 'host_decode_test.cpp', 'locked_identity.h', 'source_contract.json', 'build.py', 'launch_metadata_json.h')
...
def verify_inputs():
    ...
    pinned += [contract[k] for k in ('original_identity_lock', 'reference_recorder', 'CUPTI_runtime', 'device_code_identity_ref', 'hardware_ref', 'baseline_build_manifest_ref', 'prior_rejected_runtime_ref')]
    pinned += contract['baseline_sources_ref']
    ...
    for filename, required in {
        'generated_cuda_runtime_api_meta.h': ('cudaLaunchKernel_v7000_params', 'cudaLaunchKernelExC_v11060_params', 'const cudaLaunchConfig_t *config', 'void **args', 'cudaStream_t stream'),
        'cupti_callbacks.h': ('functionReturnValue', 'symbolName', 'functionParams', 'cuptiEnableDomain'),
        'cupti_runtime_cbid.h': ('CUPTI_RUNTIME_TRACE_CBID_cudaLaunchKernel_v7000','CUPTI_RUNTIME_TRACE_CBID_cudaLaunchKernelExC_v11060'),
...
def build():
    ...
    manifest=dict(schema='heterollm.mmvq-target-capture-build/v2',status='compiled_not_GPU_executed',
...
def host_test(manifest_path):
    ...
    attempt=1+len([p for p in HERE.glob('host_test.*.json') if re.fullmatch(r'host_test\.\d{4}\.json',p.name)])
