...
static std::string launches_json(const Recorder & rec) {
    ...
    for (unsigned i=0; i<count; ++i) {
        ...
        s << "{\"index\":" << i << ",\"api_id\":" << r.api_id << ",\"api_name\":" << quote(r.api_name)
          ...
          << ",\"function\":" << r.function << ",\"stream\":" << r.stream << ",\"grid\":" << vector_json(r.grid)
          << ",\"block\":" << vector_json(r.block) << ",\"shared\":" << r.shared
...
int main(int argc, char ** argv) {
    ...
    try {
        ...
        auto *w=ggml_new_tensor_2d(ctx,GGML_TYPE_Q5_0,kK,kN); auto *x=ggml_new_tensor_2d(ctx,GGML_TYPE_F32,kK,kM);
        ...
        valid=validate(rec,tensors,reason);exitcode=valid?0:4;
    ...
    result<<"{\"schema\":\"heterollm.mmvq-target-launch-capture/v1\",\"status\":"<<quote(valid?"qualified_runtime_pair":"rejected")
          ...
          <<",\"hardware\":"<<hardware<<",\"configuration\":{\"format\":\"Q5_0\",\"type_id\":6,\"M\":1,\"K\":4096,\"N\":3072,\"seed\":20260916,\"ids\":false,\"fusion\":false,\"graph_replay\":false}"
