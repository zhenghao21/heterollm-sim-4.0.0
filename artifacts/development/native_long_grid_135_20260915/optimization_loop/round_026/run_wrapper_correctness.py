...
reviewed campaign closure, idle processes, immutable target-capture prerequisite,
...
def verify_build():
    ...
    return {"inputs": inputs, "build": build, "static": static, "host": host,
            "prerequisite": prereq, "hardware": read_json(HARDWARE),
...
def capture_hardware(label, output, env, expected):
    ...
    snapshot = {"schema": "r26-wrapper-hardware-snapshot/v1", "created_utc": now(), "tool_ref": ref(SMI),
                "commands": commands, "gpu": gpu, "compute_processes": compute,
    ...
    require(not compute, "another CUDA compute process is present: " + json.dumps(compute))
    return snapshot
...
def compare_hardware(before, after, device, expected):
    ...
    require(not before["compute_processes"] and not after["compute_processes"], "non-idle CUDA process inventory")
    ...
    return {"same_device_and_driver_before_after": True, "raw_CUDA_matches_frozen_static_properties": True,
            "performance_stability_qualified": False}
...
def execute():
    ...
    assert_idle()
    output = P / "wrapper_correctness_run.0001"
    output.mkdir(exist_ok=False)
    result_path = output / "correctness.json"
    finish = {"schema": "r26-wrapper-correctness-finish/v1", "status": "rejected", "returncode": None,
    ...
    try:
        ...
        write_new(output/"start.json", {"schema": "r26-wrapper-correctness-start/v1", "created_utc": now(),
...
def main():
    ...
    if args.check:
        ...
        print(json.dumps({"status": "frozen_wrapper_build_verified", "gpu_execution_performed": False,
                          ...
                          "execution_requires_closed_campaign_idle_and_hardware_checks": True}))
    else:
        execute()
