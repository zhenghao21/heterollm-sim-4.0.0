...
import pytest
spec=importlib.util.spec_from_file_location('r26_gate_test',Path(__file__).with_name('run_target_capture.py'));d=importlib.util.module_from_spec(spec);spec.loader.exec_module(d)
...
def test_startup_error_preserves_failed_terminal(monkeypatch,tmp_path):
    ...
    import json
    finish=json.loads((tmp_path/'target_capture_run.0001/finish.json').read_text())
