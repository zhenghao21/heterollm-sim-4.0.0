...
with p.open('x',encoding='utf-8') as out:json.dump(contract,out,indent=2)
freeze_selection(Path(f['selection_ref']['path']),R/'tail_candidate',data_root=Path(f['data_root']),model_snapshot_map={k:v['path'] for k,v in f['model_snapshot_map'].items()},runtime_build_audit_path=Path(f['runtime_build_audit']['audit_ref']['path']),recurrent_batching_contract_path=Path(f['recurrent_batching']['contract_ref']['path']),slot_order_contract_path=Path(f['slot_order']['contract_ref']['path']),host_offload_source_contract_path=Path(f['host_offload_source']['contract_ref']['path']),tensor_storage_contract_path=Path(f['tensor_storage']['contract_ref']['path']),tensor_storage_f32_hidden=True,gpu_invocation_contract_path=p,gpu_mmq_source_costs=True)
print('R14 tail contract/candidate frozen; prior freezes retained')
