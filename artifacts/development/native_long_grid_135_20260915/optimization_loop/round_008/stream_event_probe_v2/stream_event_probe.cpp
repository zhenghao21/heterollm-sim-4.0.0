...
struct Options {
    ...
    int64_t m=0,n=0,k=0;int batch=64,threads=1,cuda_index=0,warmup=5,repeats=30,samples=4096;
...
static Options parse(int argc,char **argv) {
    Options o;for(int i=1;i<argc;++i) {
        ...
        if(a=="--help"){std::cout<<"stream-event-probe --device cuda --m 1|2|4 --n 4096 --k 896|1024 --quant Q5_0|Q8_0 --batch 64 --threads N [--cuda-index N] [--check-only|--run] [--nvtx] [--evict-mib 0|128] [--warmup 3 --repeats 20 --samples 32 --seed 20260914 --atol .05 --rtol .03 --output file.json]\nUse invoke.ps1 without -Run for identity-only checks without device access. Binary check-only performs GPU backend support checks.\n";std::exit(0);}
    ...
    if(o.batch!=64||o.threads!=1||o.warmup!=5||o.repeats!=30||o.samples!=4096||o.seed!=20260914||o.atol!=0.05||o.rtol!=0.03||o.evict_mib||o.nvtx||o.cuda_index!=0) throw std::runtime_error("protocol override forbidden");
    return o;
}
...
int main(int argc,char**argv) {
    ...
    try{options=parse(argc,argv);status=run(options,result);}catch(const std::exception&e){result="{\"schema\":\"backend-stream-event-probe/v2\",\"status\":\"error\",\"error\":"+quote(e.what())+"}";}
    std::cout<<result<<'\n';
