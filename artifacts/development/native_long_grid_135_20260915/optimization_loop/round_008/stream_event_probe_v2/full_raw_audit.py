...
def audit_run(doc,config,mode,protocol,manifest=None):
    ...
    def check_numerics(check,label):
        ...
        for j,sample in enumerate(samples):
            ...
            a,r=sample.get("actual"),sample.get("reference")
            if type(a) not in (int,float) or type(r) not in (int,float) or not math.isfinite(a) or not math.isfinite(r) or sample.get("pass") is not True or abs(a-r)>0.05+0.03*abs(r):issues.append(label+":numerical sample rejected");break
    ...
    for i,row in enumerate(rows):
        label=f"row {i}"
        if row.get("graph_computations")!=64:issues.append(label+":submission count")
        ...
        if positive(frequency) and valid:
            ...
            for key,want in [("host_wall_ns",wall),("host_per_graph_ns",wall/64)]:
