...
$required=@('stream_event_probe.cpp','stream-event-probe.exe','protocol.json','invoke.ps1','run_frozen_matrix.ps1','assess.py','full_raw_audit.py','summarize_matrix.py','identity_lock.h','identity_lock.json')
...
$c=$chosen[0]
$argv=@('--run','--device','cuda','--quant',$c.quant,'--m',"$($c.M)",'--n',"$($c.N)",'--k',"$($c.K)",'--batch','64','--threads','1','--cuda-index','0','--warmup','5','--repeats','30','--samples','4096','--seed','20260914','--atol','0.05','--rtol','0.03','--output',$Output)
