"""Explicit-reference R33 packet. No recursive campaign discovery, score or prediction.
...
def select(root,heatmap_dir='heatmaps.0001'):
    ...
    need(barrier['schema']=='r33-full262-terminal-barrier/v1' and barrier['terminal_count']==262
    ...
    need(report['schema']=='r33-two-arm-scoring-receipt/v1' and report['denominator_per_arm']==131
    ...
    need(grouped['schema']=='r33-grouped-paired-errors/v1' and grouped['fixed_cells']==131
    ...
    for arm in ('off','on'):
        ...
        for stage,mapping in [('freeze',receipt['frozen_source_preflights']),('lock',controls['lock_preflights'])]:
            ...
            need(pre['schema']=='r33-frozen-source-preflight/v1' and pre['stage']==stage and pre['arm']==arm
    ...
    need(heat['schema']=='r33-presentation-only-heatmaps/v1' and heat['fixed_cell_denominator']==131
...
def package_selected(root,output,members,part_size=40*1024*1024,final_verifier=None):
    ...
    finish={'schema':'r33-reference-selected-archive/v1','status':'rejected','created_utc':now(),
...
def main():
    ...
    spec=importlib.util.spec_from_file_location('r33_archive_postprocess',HERE/'postprocess.py')
