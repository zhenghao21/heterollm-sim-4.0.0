"""R33 two-arm full262 terminal barrier; no scoring before both arms finish."""
...
spec = importlib.util.spec_from_file_location("r33_freezer", P / "freeze_candidate.py")
...
def read_freeze_receipt():
    ...
    s.require(receipt.get("schema") == "r33-paired-freeze-receipt/v1" and receipt.get("compared_cells") == 131
...
def guard():
    ...
    s.require(controls.get("schema") == "r33-two-arm-controls/v1" and controls.get("arms") == list(ARMS)
...
def lock():
    ...
    f.write_new(P / "controls.json", {"schema": "r33-two-arm-controls/v1", "created_utc": now(), "arms": list(ARMS),
...
def barrier():
    ...
    s.require(record.get("schema") == "r33-full262-terminal-barrier/v1" and record.get("terminal_count") == 262
...
def full(workers, timeout):
    ...
    f.write_new(P / "predictions_complete.json", {"schema": "r33-full262-terminal-barrier/v1", "created_utc": now(),
...
def score():
    ...
    if target.exists():
        ...
        return
    f.write_new(target, {"schema": "r33-two-arm-scoring-receipt/v1", "created_utc": now(), "barrier_ref": bref,
...
if __name__ == "__main__":
    ...
    try:
        ...
        f.write_new(P / (args.phase + "_rejected.%04d.json" % (len(list(P.glob(args.phase + "_rejected.*.json"))) + 1)),
            {"schema":"r33-phase-failure/v1","created_utc":now(),"phase":args.phase,"status":"failed",
