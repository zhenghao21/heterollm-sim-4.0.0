"""R23 two-way saved-evidence report; standard library only.
...
VARIANTS=('current','retained')
CANDIDATE='retained'
TREATMENTS={v:{'mmq':True,'sampling':True,'nonflash':True,'cta':True,'issue':True,'retained':v=='retained'} for v in VARIANTS}
DECLARED_VARIANTS={'current':'R22 CTA+issue mechanisms; retained state off',
    'retained':'same mechanisms plus conditional retained warmup state; hybrid retains original fallback'}
...
def source_content(freeze):
    ...
    for required in ("src/heterollm_sim/config.py", "src/heterollm_sim/planner.py",
                     ...
                     "tools/native_llama_compare.py", "tools/retained_warmup_extractor.py"):
...
def normalize_document_path(value,document_ref):
    if isinstance(value,dict):
        return {key:('frozen-identical-hardware-document' if key=='path' and isinstance(item,str) and str(Path(item).resolve())==document_ref['path'] else normalize_document_path(item,document_ref)) for key,item in value.items()}
    if isinstance(value,list):return [normalize_document_path(item,document_ref) for item in value]
    return value


def retained_group(cell):
    proof=(cell.get('static_inputs') or {}).get('retained_kv_warmup_evidence')
    require(isinstance(proof,dict),'missing retained group proof')
    scope=proof.get('model_scope',{});arch=scope.get('architecture')
    if proof.get('status')=='conditional':
        require(arch in ('llama','qwen2') and scope.get('status')=='conditional' and isinstance(proof.get('contract'),dict),'ordinary cache qualification differs')
        return 'retained_covered'
    require(proof.get('status')=='uncovered' and proof.get('contract') is None and proof.get('uncovered_reasons'),'fallback lacks explicit uncovered reason')
    return 'fallback'
...
def static_variant_semantics(bundle):
    ...
    retained=freeze.get('retained_kv_warmup')
    require(freeze.get('retained_kv_warmup_state',False) is mode['retained'],'campaign retained switch differs')
    if mode['retained']:
        require(isinstance(retained,dict) and retained.get('requested') is True and set(retained.get('cells',{}))==set(bundle['cells']),'retained evidence denominator')
    else:require(retained is None,'unexpected retained evidence')
    for ident,row in bundle['cells'].items():
        ...
        require(inputs.get('retained_kv_warmup_state',False) is mode['retained'],'cell retained switch differs')
        if mode['retained']:
            cell=retained['cells'][ident]
            require(inputs.get('retained_kv_warmup_evidence')==cell and inputs.get('retained_kv_warmup_contract')==cell.get('contract'),'cell retained proof differs')
            require(cell.get('requested') is True and cell.get('status') in ('conditional','uncovered') and cell.get('native_latency_used') is False and cell.get('native_request_times_used') is False,'retained qualification provenance')
            require(cell.get('selection_ref',{}).get('sha256')==SELECTION_SHA,'retained selection byte identity differs')
        else:require(inputs.get('retained_kv_warmup_evidence') is None and inputs.get('retained_kv_warmup_contract') is None,'unexpected cell retained proof')
        common=deepcopy(inputs)
        for key in ('retained_kv_warmup_state','retained_kv_warmup_contract','retained_kv_warmup_evidence'):common.pop(key,None)
        # Each arm stores its own identical PDF. Normalize only that verified
        # arm-local path; retain all SHA, byte counts and mechanism fields.
        document=(freeze.get('mmvq_issue_bound') or {}).get('hardware_document',{})
        if document.get('ref'):
            actual=verify_reference(document['ref'])
            common=normalize_document_path(common,actual)
        normalized[ident]=stable_hash({'static_inputs':common,'model_key':row.get('model_key'),'deployment':row.get('deployment')})
    return normalized


def check_protocol(protocol):
    require(protocol.get('schema')=='retained-two-way-ablation/v1' and protocol.get('variants')==list(VARIANTS),'two-way protocol domain')
    ...
    require(protocol.get('retained_scope_counts')=={'retained_covered':62,'fallback':69},'protocol static scope counts differ')
    require(protocol.get('prior_round_results_unchanged') is True and protocol.get('terminal_prediction_count_includes_failures') is True,'protocol legacy/failure accounting differs')
...
def check_controls(directory,phase=None):
    ...
    require(controls.get('schema')=='retained-two-way-controls/v1' and controls.get('same_source_closure') is True and controls.get('variants')==list(VARIANTS),'two-way control manifest identity')
...
def evidence_closure(freeze):
    ...
    """
    refs={}
    for name in ('runtime_build_audit','recurrent_batching','cpu_iq_panel_reuse','slot_order','host_offload_source','tensor_storage','gpu_invocation','sampling','nonflash_kv_view','mmvq_issue_bound','retained_kv_warmup'):
        binding=freeze.get(name)
        if binding:
...
def frozen_bundles(directory,protocol):
    ...
    groups=Counter(retained_group(cell) for cell in bundles[CANDIDATE]['cells'].values())
    require(groups=={'retained_covered':62,'fallback':69},'fixed static 62 ordinary / 69 fallback qualification differs')
...
def read_phase_receipt(directory,phase,kind,controls):
    ...
    require(document.get('schema')=='r23-'+('prediction-barrier' if kind=='predictions' else 'score-receipt')+'/v1' and document.get('phase')==phase,'phase receipt schema')
...
def assess(directory,phase):
    ...
    # Both terminal anchor sets were validated before a score is read.
    ...
    require(latest<=earliest and timestamp(barrier['created_utc'])<=earliest,'scoring began before both anchor prediction sets and barrier finished')
    ...
    comparisons={name:compare(rows[a],rows[b],ids) for name,a,b in (
        ('retained_minus_current','current','retained'),)}
    full=None;full_refs=[]
    if phase=='full':
        full_barrier,full_barrier_ref,full_bundles=load_prediction_barrier(directory,'full',controls)
        require(normalized_ref(full_barrier.get('anchor_score_receipt_ref'))==score_receipt_ref,'full continuation lacks completed two-way anchor scoring receipt')
        full_receipt,full_receipt_ref=load_score_receipt(directory,'full',controls,full_barrier_ref)
        b=full_bundles[CANDIDATE]
        require(all(timestamp(b['predictions'][ident]['created_utc'])>=timestamp(score_receipt['created_utc']) for ident in b['planned'] if ident not in ids),'full continuation started before two-way anchor scoring ended')
    ...
    qualification=bundles[CANDIDATE]['freeze'].get('retained_kv_warmup',{})
    candidate_rows=full['rows'] if full else scores[CANDIDATE]['rows']
    group_ids={}
    for ident,cell in bundles[CANDIDATE]['cells'].items():
        for key in ('model:'+str(cell.get('model_key')),'scope:'+retained_group(cell)):
            group_ids.setdefault(key,[]).append(ident)
    group_reports={key:{'fixed_denominator':len(group),
        'candidate_summary':summarize({ident:candidate_rows[ident] for ident in group}),
        'anchor_comparison':compare(rows['current'],rows['retained'],[ident for ident in group if ident in ids])}
        for key,group in sorted(group_ids.items())}
    return {'schema':'r23-retained-two-way-ablation/v1','created_utc':datetime.now(timezone.utc).isoformat(),
        ...
        'anchor_comparisons':comparisons,'anchor_rows':rows,'group_reports':group_reports,'candidate131':totals,'candidate131_rows':full['rows'] if full else scores[CANDIDATE]['rows'],
        'coverage':{'completed_predictions131':len(bundles[CANDIDATE]['predictions']),
            'scored_all3_cells':sum(all(row['metrics'][m]['status']=='scored' for m in METRICS) for row in (full['rows'] if full else scores[CANDIDATE]['rows']).values()),
            'saved_predictions_as_of_phase':{v:len(bundles[v]['predictions']) for v in VARIANTS},
        ...
        'retained_static_qualification':{'conditional_cells':qualification.get('conditional_cell_count'),'uncovered_cells':qualification.get('uncovered_cell_count'),
            'actual_retained_application_count':None,'application_inferred_from_cell_switch':False,
            'scope':'62 ordinary declarations and 69 explicit fallback declarations; eligibility is not a measured timing change.'},
...
def markdown(result):
    ...
    lines=['# R23 retained KV 两路开发消融','',f"阶段：{result['phase']}。retained 全三项严格小于10%：{total['strict_all3_below10_cells']}/131；A={result['gate_A']}，B=unvalidated。新增系数0。",'',
    ...
    lines+=['','差值均为后者减前者；APE差为负表示误差下降。未评分保留在每指标20格分母内。',
        f"retained 已保存预测 {result['coverage']['saved_predictions_as_of_phase'][CANDIDATE]}/131；缺失 {result['coverage']['candidate_missing_prediction_cells']}；失败或未评分 {total['failed_or_unscored_cells']}/131。",
        f"终态结果（含failure）{result['coverage']['completed_predictions131']}/131；三项可评分 {result['coverage']['scored_all3_cells']}/131。",
        '新增机制uncovered只降低retained覆盖，不删去原数值预测。原R22的SHA失败仍属原实验失败；本轮新源码预测不得改写旧结论。']
    lines+=['','| 分组 | 固定分母 | 三项严格<10% | 失败/未评分 |','|---|---:|---:|---:|']
    for key,item in result['group_reports'].items():
        summary=item['candidate_summary'];lines.append(f"| {key} | {item['fixed_denominator']} | {summary['strict_all3_below10_cells']} | {summary['failed_or_unscored_cells']} |")
    if result['phase']=='full':
        lines+=['','retained 全131 APE：']+[f'- {LABELS[metric]}：'+cell(total['metrics'][metric]['absolute_percentage_error_pct']) for metric in METRICS]
...
def heatmap(result):
    ...
    svg=[f'<svg xmlns="http://www.w3.org/2000/svg" width="{w}" height="{h}"><rect width="100%" height="100%" fill="white"/><style>text{{font:12px sans-serif}}</style><text x="12" y="22">R23 two-way anchor APE (%) · B unvalidated · full denominator 131</text>']
