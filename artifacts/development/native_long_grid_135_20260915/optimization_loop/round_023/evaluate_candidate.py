"""R23 two-way experiment phases; never executes a native benchmark.
...
BASELINE=P.parent/'round_022/cta_issue/freeze.json'
_spec=importlib.util.spec_from_file_location('r23_saved_ablation_report',P/'summarize_ablation.py')
...
def freeze_arguments(baseline,variant,hardware_document=None):
    ...
    result=dict(data_root=Path(baseline['data_root']),
        ...
        mmvq_vector_issue_bound=mode['issue'],retained_kv_warmup_state=mode['retained'])
    if hardware_document is None:
        hardware_document=Path(baseline['mmvq_issue_bound']['hardware_document']['ref']['path'])
    result['mmvq_issue_hardware_document_path']=hardware_document
    return result
...
def freeze(hardware_document=None):
    ...
    if path.exists():
        ...
        protocol={'schema':'retained-two-way-ablation/v1','created_utc':now(),'native_lock':start,
            ...
            'workflow':'Freeze both full131 variants, lock controls, save all40 terminal anchor predictions, then score both; continue retained to fixed131 while retaining failures.',
            'continuation_criterion':'Mechanism/source identity checks; never select cells or censor failures based on errors.',
            'retained_scope':'Actual GGUF cache qualification: 62 ordinary conditional declarations; 69 hybrid fallback. No timing fit or model-key eligibility.',
            'retained_scope_counts':{'retained_covered':62,'fallback':69},
            'prior_round_results_unchanged':True,
            'terminal_prediction_count_includes_failures':True,
            'comparisons':[{'before':'current','after':'retained'}]}
...
def lock_controls():
    ...
    payload={'schema':'retained-two-way-controls/v1','created_utc':now(),'variants':list(VARIANTS),
...
def anchors(workers=4,timeout=600):
    ...
    check_end(before)
    write_new(barrier_path,{'schema':'r23-prediction-barrier/v1','phase':'anchors','created_utc':now(),
...
def score_anchors():
    ...
    for variant,bundle in bundles.items():
        ...
        s.require(actual=={ident+'.prediction.json' for ident in protocol['anchor_ids']},'full prediction appeared before both anchor scores')
    ...
    for variant in VARIANTS:
        ...
        s.require(s.timestamp(score['created_utc'])>=s.timestamp(barrier['created_utc']),'score preceded both prediction barrier')
        scores[variant]=score_ref;s.load_prediction_barrier(P,'anchors',controls);check_end(before)
    write_new(receipt_path,{'schema':'r23-score-receipt/v1','phase':'anchors','created_utc':now(),
...
def full(workers=4,timeout=600):
    ...
    if not path.exists():
        ...
        write_new(path,{'schema':'r23-prediction-barrier/v1','phase':'full','created_utc':now(),
    ...
    if not (P/'full_scores.json').exists():
        ...
        write_new(P/'full_scores.json',{'schema':'r23-score-receipt/v1','phase':'full','created_utc':now(),
