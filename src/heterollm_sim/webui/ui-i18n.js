...
(function initUiI18n(root, factory) {
  ...
  const DEFAULT_TEXT_PAIRS = Object.freeze({
    ...
    "导入 JSON": "Import JSON",
    "导出 JSON": "Export JSON",
    "导入硬件参数": "Import hardware parameters",
    "导出硬件参数": "Export hardware parameters",
    "Native 评分": "Native score",
    "关闭 Native 评分": "Close Native score",
    "对当前场景执行仿真并比较 Native 实测时延；调度与映射作为候选方案参与评估，结构差异单独展示。": "Simulate the current scenario and compare it with measured Native latency. Scheduling and placement are evaluated as part of the candidate, with structural differences shown separately.",
    "Native 参考结果（JSON，用于本次对比）": "Native reference results (JSON for this comparison)",
    "R0 参考结果（可选 JSON；只比较 R0 误差和结构）": "R0 reference results (optional JSON; compare R0 error and structure)",
    "直接仿真评分（Direct score）": "Direct simulation score",
    "设置": "Settings",
    "关于": "About",
    "保存设置": "Save settings",
