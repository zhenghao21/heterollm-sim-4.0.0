...
第13轮：增加独立只读A门执行器，从逐请求绝对时间戳重算Engine指标，校验固定native SHA、131格集合、冻结源、预测文件及评分身份；重新计算未舍入误差，核对汇总，禁止缺格、跨版本、NaN/Inf/bool和篡改通过。先后核验证据未变化。开发通过不会提升B门，程序输出后续行动。

验收器补强后78项相关回归通过：执行前后校验自身与4个实际辅助模块，内嵌状态/策略快照，后续更新任务状态不改变旧判定证据；同一格的明确超标与证据不足可同时计数。覆盖磁盘131格证据链、阈值、文件篡改和中途代码变更。R6固定结果重算：9/131格三项通过、120格明确超标、2格证据不足，82/393项通过。总判定精度失败，同时保留证据失败计数。第13轮未运行native或修改成本模型。
...
1. 目标审计保留每格三项严格<10%，拆为A固定集开发门和B独立正式门；131格全部为3批次初筛，不能将A门直接当正式停止条件。当前两门均未达标。
2. 第13轮已实现A门执行器，16项专属回归通过；R6全量重算为9格通过、120格精度失败、2格证据不足，B未验证。第14轮已完成MMQ K896尾段建模：逻辑K、转换K与执行K分别记录，stream-K按逻辑量化块分配，已对齐896组合旧字段不变。51项联合回归通过；20格（Qwen2.5全部17格+3模型控制）新冻结正在预测，未读取新LLM实测。独立审查并行，后续仍按结果决定保留。
3. 第15轮新版Nsight2026.5.1已完成兼容trace与直接对照：24个算子区间关联72个GEMM链kernel，兼容和结构门通过；kernel并集中位15.6485us、直接主机48.6us、profile主机56.5us。单组顺序对照+16.3%不能认定为纯观测开销，不生成成本系数。下一轮准备多shape独立算子证据；采集与仿真重负载串行，不能把旧不兼容trace提升为校准。继续独立算子联合域、实际大权重/缓存及runtime路径证据；12组长窗口只通过诊断门，不能直接替换孤立kernel成本。改进必须向仿真最坏格误差或必要证据缺口传递。
4. 形成合格成本候选后做纯分析/现有/新机制消融，继续复用固定native，不新增场景常数；A门通过后再冻结独立正式验收计划及统计方法，不重用已揭示的K1024或固定131格作盲测。
5. 每轮原位更新任务书并commit+push；阶段轮数仅复盘点，未达标自动续轮。连续3轮无误差或必要证据进展时自动复盘路线，无法推进才报告外部或研究阻塞，不进行无界同方案重试。
