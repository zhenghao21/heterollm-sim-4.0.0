...
>
> **当前目标**：固定131格开发回归范围内，每格Engine TTFT、TPOT、E2E绝对相对误差均严格小于10%；阶段轮数仅为复盘点，未达标自动续轮。当前尚未达标，独立泛化验收仍另行进行。
...
|---|---:|---:|---:|
| 每格三项绝对相对误差（正式停止标准） | 每格TTFT_engine、TPOT_engine、E2E_engine均严格<10%，等于10%不通过 | 单独报告各边界，不设抵销一级的阈值 | 单独报告，不设抵销一级的阈值 |
...
当前固定范围为stable_native_dataset.json锁定的131格，不能因预测困难再缩小；原始162格和31格稳定性排除仍单独完整报告。逐格误差按各场景重复运行中位数计算，使用未舍入数值判定；不要求每次重复的瞬时时延误差都小于10%。必须在同一冻结版本获得131/131格、393/393项有效且严格<10%的结果，失败、超时、身份异常、非有限值和证据不足均不能通过。固定集通过只代表开发回归目标达到，不代表独立盲测或跨硬件通过。
...
10. 每轮提交推送后复核第7节逐格三项目标；未达到且仍有可执行方向时自动进入下一轮，无需用户再次发送“继续”。阶段轮数和计划时间窗仅为复盘检查点，记录阶段效果和下一阶段预算后自动续轮，不作为停止理由。连续多轮仅完善工具而未改善预测时，必须复盘机制假设和证据缺口，避免无效重复；只有目标通过、用户明确暂停或现有授权与资源下无法解决的外部阻塞才停止，仍能独立完成的工作继续。不得突破用户明确的费用、资源或时间硬上限。
...
缺少第二种硬件或独立验收数据时，交付可执行协议并明确“未验证”。阶段轮数或计划时间窗到达时保存检查点并自动登记下一阶段，未达标继续。只有用户明确的硬预算/资源限制或不可解决的外部阻塞才暂停依赖工作并交付可恢复状态；不能放宽阈值、隐藏失败或把规格参数冒充实测验证。最终结论只允许写“通过、失败、尚未验证”三类与证据相符的状态。
...
第9轮只读源码参考诊断：Q5_0六组重建权重/输入SHA全部一致；源码Q8_1激活量化与半精度原输入和修正后的参考最大绝对差1.15–1.34e-6，原F32数学参考差约0.067–0.070。支持参考路径遗漏的机制解释，非GPU准确度或仿真时延达标证明；旧失败不改写。第10轮已实施双参考并完成全部24进程，解决数值提前退出；时延标定仍被8组计时拒收阻断，第11轮已分解全部12组×30批次：提交占主机中位数23%–40%，与设备包络相关性多数组较弱，不能归因单一CPU提交。第12轮1024次长窗口已完成：12/12诊断准入、24进程证据完整，设备P90/P10为1.11–1.26，最大偏离10.18%–21.42%，未达±5%。17项回归和73项身份通过，无成本系数生成。第二窗口预设6轮（7–12）完成，整体准确性失败；并非8小时时间用尽。
...
1. 第二窗口6轮（7–12）已完成；用户现已明确以每格三项<10%为目标，原阶段预算停止规则被自动续轮规则替代，任务尚未达标。第7轮2格补预测均成功但误差仍失败；未混合新旧冻结或重测LLM。
2. 保留长窗口双参考方法：12组诊断准入、完整性100%，但最大波动仍10.18%–21.42%。这不是±5%稳定性或孤立kernel标定，不能直接把均摊包络塞入成本模型。
3. 下一窗口先扩展独立算子联合域：实际大N/权重工作集、热/冷缓存、MMVQ/MMQ路径与调度供给状态，预先划分新验证组。当前K1024已揭示，仅作开发回归。
4. 核对源码/实际runtime构建等价与dispatch证据；保留数学参考与量化路径参考两路误差。不将历史数值失败追认为通过，不放宽旧阈值。
5. 合格底层证据形成候选后再做纯分析/现有/新机制消融，继续复用固定native；GET_ROWS、KV与backend复制/同步仍需单独证据。
6. 每轮原位更新任务书，完成验证后commit+push。新正式验收必须独立；目前没有全分组精度或跨硬件通过结论。
...
- 第一窗口6轮机制、3次完整评估已结束并归档。第二窗口于2026-09-15T22:22:41.555440+00:00开启，截止2026-09-16T06:22:41.555440+00:00；以第7轮和累计3次完整评估为起点；已完成7–12共6轮，该阶段已结束，仅作复盘点；按第8节第10条自动续轮，旧冻结不改写。
- 第六轮独立r2冻结保留原合同、三路12锚点消融、72静态图核验与三个控制逐请求精确一致证据。2格SHA失败记录不可改写或算通过。
- 独立launch诊断480正式样本原始记录有效，但16组均未过全部稳定性门，未生成系数。stream事件探针第二窗口完成12配置、0通过；第8/10/12轮准入分别2/12、4/12、12/12，全部历史保留；未改变成本模型。
