...
3. 新版Nsight2026.5.1已在项目内解包并核验NVIDIA签名，准备CUDA13.4兼容trace及无profile对照，尚未运行；采集与仿真重负载串行，不能把旧不兼容trace提升为校准。继续独立算子联合域、实际大权重/缓存及runtime路径证据；12组长窗口只通过诊断门，不能直接替换孤立kernel成本。改进必须向仿真最坏格误差或必要证据缺口传递。
