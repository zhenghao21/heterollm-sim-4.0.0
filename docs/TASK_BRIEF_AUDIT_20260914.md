...
1. R24已完成131终态、评分及收尾身份检查：130预测、1个完整GGUF SHA失败保留；9/131格三项严格<10%，A未通过、B未验证。原R23的104格精确不变，原27失败恢复26格，R22共同25GPU格中24格精确相同、1格本轮失败。此轮仅恢复执行覆盖，不是成本改善。归档结果后进入R25两路同源选行评估；不重试覆盖SHA失败。
2. 已确认末层output-selection生产接线缺失：真实GGUF架构名qwen2/llama与resolver别名不一致，metadata嵌套位置与planner读取位置不一致。隔离R25候选已实现默认关闭绑定，补充physical-dispatch结构回归后34项通过，分支7c7e51e已上传并核对；普通completion配置源码链已审查，候选已窄合入主区，保留R24身份修复，主区联合回归114通过、1跳过；默认关闭，两路同源freeze入口已准备并通过6项防错测试及131格真实静态预检，只允许选行开关与对应证明、extractor复制位置及经重新核验的retained派生摘要变化；运行器已补齐两路共262终态屏障、评分后禁止恢复预测、已有终态引用保护，冻结与运行器共9项测试通过；R25两路131冻结已生成；初次比较因硬件PDF复制路径不同失败并保留，经精确字段/内容校验取得独立qualification凭据，尚未lock或预测，不改变R24冻结副本。静态探针确认attention保留M64、末层FFN重派生M1，down进入MMVQ，融合up/gate仍保留未覆盖标记；qwen2/llama最后FFN前选行与混合架构最终norm后选行分开，绑定实际GGUF结构和锁定源合同，检查真实算子shape。独立候选与消融；语义修正可能降低prefill成本，不能承诺改善当前偏低TTFT。
