...
2. 优先实现与任务书一致的验收执行器：锁定范围/版本、逐格393项、未舍入阈值、缺失/身份/非有限值/证据不足拒收，明确A/B和131/162覆盖分母；现有state目标只是记录字段。
