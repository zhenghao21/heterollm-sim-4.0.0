...
final norm设备/算子粒度修复与旧输出索引放置逻辑存在可复现的结构冲突：两路未改动的R33冻结源码在两层qwen3_5架构小图中，M64 prefill的索引与GET_ROWS消费者均在GPU；M1 decode的消费者转到CPU，但索引仍预先按旧final_norm.apply规则发往GPU，绑定依赖时抛出StopIteration。该复现不读取GGUF或目标时延。R33/off的20个27B CPU格同样记录StopIteration，但正式worker未保存异常栈，不能据此认定20格均已逐一证实同一调用链。先按实际消费者修正索引放置和依赖，不能吞异常或按模型名称禁用GPU。非MTP在线target的GPU消费者与提交/传输归属已完成148项结构回归，准确性仍待R33对照；逐ubatch的CPU生产—GPU消费关系及CPU图输入兼容性、重分配条件仍部分未知。这些是已有源码和任务图指向的候选缺口，不把系统性偏低直接归因于其中任何一项。MMVQ访存采用MMA输出tile效率的族混用保留为独立成本假设。已有nonflash、logits、sampling、launch不能整段再加；kernel_launch已归GPU frontend。P128/512/1536均整除ubatch64，不能默认有尾块；c不等于kernel M。

## 13. 当前执行及下一轮优化顺序（动态，原位更新）

1. **完成正在运行的R33，先保存完整对照。** 两路共用固定131格native、成本及配置，唯一仿真源码差异为planner；四份真实freeze/lock预检均131格通过。当前off已自然完成131终态（111预测、20失败），on已开始，沿用同一会话62083继续等待。全部262终态封存后才评分、分组、严格A门检查和作图；冻结运行期间不改影响结果的源码/数据，不重试失败格。R32准备拒收与R33首次启动拒收均保留；512次摘要诊断和一次同次读取guard未复现异常，只支持已记录的一次原版启动恢复，不证明根因已消失，不再自动重试身份失败。
2. **下一版本首先修正输出索引的真实消费者绑定。** 第12节的小图已把“有事件但设备与依赖连接错误”与局部服务成本区分开。当前版本结束后，让索引准备、每目标唯一传输和GET_ROWS依赖以实际消费者为准，保留每物理invocation发生次数，避免旧final_norm.apply预判。按静态执行路径预先规定受影响集合，不按20个失败格的模型名称加例外。覆盖CPU/GPU与split、M1/2/4及31/32/33/64边界、before_last_ffn/after_final_norm和模板重放；后续失败凭据保存原始异常栈。修复另建冻结，不覆盖R33记录。
3. **围绕剩余误差检查系统关系，再决定下钻位置。** 核对实际CPU准备→提交→GPU消费→同步/回传→采样→下一token链、资源owner及释放时机；逐ubatch生产消费和buffer复用需有调用次数与依赖证据。CPU侧can_reuse与CUDA graph replay分开，先核对检查/复用/重建/重分配触发和发生频率，再为事件定价。CPU图状态机已接serving且模板cache不跳过转换，但warmup末图、完整input roster、精确KV视图及部分同步仍unknown/partial/unpriced，不把“已记录状态”当作完整有价DAG。
4. **按真实适用范围比较结构与成本假设。** 保留已有readback→通知→采样→commit链及GPU frontend的kernel_launch owner，检查等待与包含关系，不能把API wall、kernel wall和stage wall重复相加。多资源共同启动是否合理取决于上游任务粒度，不能直接修改调度器作为默认修复。R27保存记录可见单组18115批、双组775批、三组34批及2947批未知，仅为仿真线索；普通单组模型的偏差不能由多组串行机制一概解释。保留跨cohort重叠，不因graph miss加全局fence；CPU核心预算的误差因果未证，不无依据扩资源系统。当前前端修复只覆盖非MTP在线target，MTP/静态request和逐ubatch时序另核对。
5. **仅对已明确的局部服务对象启用底层证据。** 若关系和次数已得到相关证据、剩余偏差指向成本，再执行R31同源Q4 wrapper资格与预定100进程/1200秒微基准；69项主机回归不等于实际GPU路径或性能通过。静态shape命中45格不证明cache/layout或性能迁移，M4和K边界仍留出，旧CPU IQ smoke不能替代27B资格。R30的freeze/lock不改，尚未执行的262预测只回答旧结构下MMVQ HBM折扣假设。R28的15.625ms粒度和长窗回压不得直接转成每kernel微秒常数。性能采集与当前完整仿真串行。
6. **更新结论、提交推送并自动续轮。** 当前最新完整误差结果仍R27/on：9/131格、74/393项通过，共同129格两路均仅7格全过；覆盖恢复不等于精度改善。R33尚未评分，不提前宣布结构修复降低了误差。每轮按第8节对照结果决定准入、原位更新任务书、commit、push并核对远端；固定131格/393项全部严格<10%前A不通过，A通过后另做独立B，当前B未验证。诊断自顶向下不增加任何验收门槛。
...
- optimization_loop/round_033/：前端结构对照的新驱动，两路共用审核过的身份诊断补丁，逐文件锁定Git来源；73项合成回归与独立复核、四份131格真实freeze/lock预检通过；startup_identity_observation.0001.json保留启动guard的小PDF身份失败；startup_resume_decision.0001.json记录一次有界恢复；off已131终态，on运行中。diagnostic/output_index_consumer_repro.py及两路JSON保留小图的索引目标/实际消费者和异常栈；不等同于正式worker逐格栈。
