...
第9轮只读源码参考诊断：Q5_0六组重建权重/输入SHA全部一致；源码Q8_1激活量化与半精度原输入和修正后的参考最大绝对差1.15–1.34e-6，原F32数学参考差约0.067–0.070。支持参考路径遗漏的机制解释，非GPU准确度或仿真时延达标证明；旧失败不改写。下一版必须同时保存数学误差和同路径数值误差，再按新冻结采完整批次。
