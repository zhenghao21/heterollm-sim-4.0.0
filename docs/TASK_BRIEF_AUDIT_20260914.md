...
R27的两个off失败（qwen38_p512_o128_c2、qwen38_gpu_p1536_o128_c4）及R24历史SHA异常保持，不重试覆盖或追认；根因未定。独立同块OpenSSL/CNG诊断0003已执行串行一遍：14177个数据块与整文件SHA均一致，文件状态不变。随后因三次完整262证据核验超过辅助读取2GiB预算而拒收，四进程阶段未启动；旧attempt拒收保留。0004正根据真实引用大小与检查次数重新预声明预算，不能将预算修复称哈希问题已解决。
...
4. 分开推进两条可改变预测的机制：完整MMVQ源契约上的执行K账本补全及去除MMA带宽折扣的默认关闭分析候选；锁定DLL宿主计数/提交控制0004通过33项Python及C++主机测试，先取得有界pilot与实际路径证据。名义带宽候选不是测量过的MMVQ带宽，不能把CTA≥SM称为饱和；条件经验值只能命中完整适用键，cache未知不得补填。每kernel frontend、driver queue、GPU command processor、CPU command-build逐项查owner，不能用一段API wall同时更新多个费用。
