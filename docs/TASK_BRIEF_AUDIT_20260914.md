...
第六轮physical_mapping_mmq_r2已结束，129/131格完成评分，2格因模型副本SHA校验不一致拒收；有效预测覆盖98.47%，原始162格分母保留。逐格三项Engine误差同时<10%为9/131，六个主要分组均未同时通过。当前是有来源约束的条件机制研究候选，未获准确性晋级。

固定native选择、131 raw和894正式请求SHA核验仍通过。运行后完整读取27B副本SHA与冻结值一致，但不能据此删除两次历史不一致或宣称根因已解决。当前固定集已揭示，仅属开发回归；没有独立盲测或跨硬件验收通过。
...
- 第六轮物理GPU投影、融合和可选MMQ成本三路已冻结，147项结构检查通过。三个关闭开关的控制格逐请求和聚合时延与第五轮完全一致；批次只增加诊断字段，旧字段完全一致。三路12锚点已完成；关闭开关的全部12场景逐请求与第五轮精确一致。四项边界/披露问题已修复，72个静态图非metadata任务与成本完全一致、315个冻结SHA未变；150项联合回归通过，2项旧合同测试触发拒收后改为验证新派生/旧合同拒收，两项均通过。修订r2已独立冻结，三控制格逐请求、聚合及MMQ计数与旧版精确一致，第3/4次完整131格评估已结束（129成功、2身份拒收），不覆盖旧预测。历史模型专用include哈希缺失，合同维持conditional和native_dispatch_proven=false。
...
1. 本轮先完成129格评分、2格拒收原因调查、任务书与报告的更新、本地提交和GitHub推送。旧预测和失败不覆盖。
2. 在新实验预算明确前不启动追加模拟或原生测量。已有6轮机制与3次完整评估保留；8小时窗口已结束，不因误差未达标自动扩大预算。
3. 下一轮首先查明27B读取SHA瞬时不一致，保持身份门禁；当前一次重新读取匹配不能替代根因。
4. 随后优先做实际ggml stream通用算子测量，覆盖M1/2/4 MMVQ、K896 Q5_0/Q8_0及对齐MMQ对照。已编译12配置stream event试点尚未运行，须另设窗口、保留控制与全部原始边界。事件包络不是纯kernel时间。
...
- 6轮机制、3/4次完整评估已执行，预定8小时窗口于2026-09-16 05:27:44结束。本轮进入结果核验交付，不再启动新实验。
- 第六轮独立r2冻结保留原合同、三路12锚点消融、72静态图核验与三个控制逐请求精确一致证据。2格SHA失败记录不可改写或算通过。
- 独立launch诊断480正式样本原始记录有效，但16组均未过全部稳定性门，未生成系数。stream事件探针仅编译未测，预算内未完成的证据明确列出。
