...
第7轮已完成身份加固与两格补预测：GGUF元数据与SHA改为同一个打开句柄读取，并验证前后设备/文件ID、长度、mtime和实际读取字节数。Windows fstat/stat的ctime存在接口语义差异，不用于跨接口内容身份判断。正常输入的哈希和模型结构不变；这只补齐身份检查缺口，不宣称解释历史SHA异常。6项可移植回归及3项独立合成检查通过。新冻结两格均预测成功；P128/O256/C1三项误差8.497%/17.727%/17.001%，P1536/O256/C2为19.506%/8.231%/10.400%，均未三项同时达标。新旧版本不合并成同一冻结。

独立串行三次27B完整读取的SHA均匹配，分块证据保留；原诊断将跨接口ctime差异标记失败，须用附加分析解释，不能覆盖原记录。实际stream事件12配置试点已完成，0/12通过原冻结门限；11组event wall波动拒收、9组control wall波动拒收、5组扰动拒收，validation Q4_K M64两模式首次计算数值失败。全部失败保留，未生成成本系数。事件/control等待方式不一致是解释限制，下一版需统一并使用预先固定的成批测量协议。
...
1. 本轮129格评分、2格拒收调查和提交推送已完成（bd729aa，远端SHA一致）。旧预测和失败不覆盖。
2. 用户明确续推后开启第二个有界窗口：最多6轮机制修改、4次完整评估、8小时；旧预算与结果保留在state.budget_history。当前先完成第7轮读取诊断和独立算子证据，不新增目标LLM实测。
3. 下一轮首先查明27B读取SHA瞬时不一致，保持身份门禁。独立审计未发现共享digest/cache；两个失败在02:06与02:10、各约10秒完成，非超时。当前一次重新读取匹配不能替代根因；先做串行只读hash诊断并记录读取字节数、前后文件身份和分块摘要，再在新版本补两格，保留原失败。
4. 随后优先做实际ggml stream通用算子测量，覆盖M1/2/4 MMVQ、K896 Q5_0/Q8_0及对齐MMQ对照。12配置stream event试点已测且全部拒收；下一版采用相同等待方式、固定成批次数，另查数值失败，保留旧失败。事件包络不是纯kernel时间。
5. launch已经按分析值收费，不能用一个常数填补全部误差。GET_ROWS/KV转换和backend split复制/同步仍需独立证据，严禁拟合目标LLM端到端时延。
6. 每轮只更新动态章节；验证完成后提交推送。新的验收集必须独立，既有固定集继续作为回归对照。
...
- 独立launch诊断480正式样本原始记录有效，但16组均未过全部稳定性门，未生成系数。stream事件探针第二窗口完成12配置、0通过；下一版尚在构建，不能提前宣称标定有效。
