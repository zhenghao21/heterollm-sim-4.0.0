...
第9轮只读源码参考诊断：Q5_0六组重建权重/输入SHA全部一致；源码Q8_1激活量化与半精度原输入和修正后的参考最大绝对差1.15–1.34e-6，原F32数学参考差约0.067–0.070。支持参考路径遗漏的机制解释，非GPU准确度或仿真时延达标证明；旧失败不改写。第10轮已实施双参考并完成全部24进程，解决数值提前退出；时延标定仍被8组计时拒收阻断，下一步分解提交/设备/等待波动，不删除慢样本。
