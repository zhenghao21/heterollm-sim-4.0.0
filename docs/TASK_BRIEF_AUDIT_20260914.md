...
第16轮采用3个子代理并行。r3探针显式加载和核验四个DLL；采集器补齐冻结集合、失败停止、额外GPU活动、异常进程收尾及2400±30MHz实际读回。第一次新会话结束于25/234阶段：8/78完整pair、0/26质量准入；一个profile计算完成后约498秒才正常退出，控制器恢复频率，旧数据保留且不并入新会话。控制器已修复检查点后等待同一进程退出并自动续跑，13项模拟状态测试通过。

当前collection_r3重新冻结1670项输入，全26配置不删样本；预注册顺序把MMQ、留出和对齐控制前移。首个MMQ pair数值/实际链/时钟通过，设备并集中位14.4325微秒，但主机波动和扰动门失败；完整矩阵继续，零profile晋级。已出现Q8_0对齐控制数值参考不一致，保留失败并独立查源，不放宽容差。

成本解析器v2已补原始SQLite/数值/频率/冻结重算、三进程去重、额外活动拒收、真实资源上下文及大raw限制，141项完整测试通过。真实trace又发现API版本后缀与Info诊断误拒收，已最小修复、13项定向测试通过；真实首pair可以解析但质量拒绝不变。默认尚未启用成本校准，未宣称降低LLM误差。26shape分析预测在有效采集前保存，80项主机测试通过。
...
2. 使用r3探针、collection_r3冻结和2400MHz同会话控制记录，串行执行26shape、每shape三对profile/direct交错对照。先验证首pair的真实kernel链和数值路径，再接续矩阵；GPU计时期间不运行重仿真或编译。硬件读回不符、外部GPU活动重叠和高扰动记录拒收且保留分母。
