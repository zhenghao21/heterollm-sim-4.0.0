...
以下为使用示例；本任务采集权限仍以任务书为准。运行 native harness 时设置环境变量：
...
最后一个节点沿用 graph 末尾的既有 barrier。单独的 operator wall 校准不能替代内存流量和带宽证据；
应按 quantized 权重实际读取字节核对 CPU memory demand。
