...
json server_slot_stats::to_json() const {
    ...
    base["engine_clock"] = "ggml_time_us_monotonic";
    base["engine_request_begin_us"] = t_start;
    base["engine_prompt_last_us"] = t_prompt_last;
    base["engine_last_token_us"] = t_gen_last;
    base["engine_token_times_us"] = engine_token_times_us;
    base["engine_timepoints_complete"] = n_gen > 0 && engine_token_times_us.size() == n_gen;
    // Speculative groups are not expanded into invented per-token times.
    base["engine_timepoints_scope"] = "non_speculative_generated_tokens";
