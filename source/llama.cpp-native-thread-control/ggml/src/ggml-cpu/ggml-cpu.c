...
// Opt-in operator wall markers for CPU-only parity runs.  The default remains
// off so ordinary llama.cpp execution and its synchronization are unchanged.
static bool ggml_cpu_operator_trace = false;  // initialized once in ggml_cpu_init()

static void ggml_cpu_operator_trace_log(
        const struct ggml_tensor * node, int n_threads, int graph_id, int node_n,
        int fused, int64_t start_us, int64_t end_us) {
    if (!ggml_cpu_operator_trace || node == NULL) {
        return;
    }
    const struct ggml_tensor * src0 = node->src[0];
    const struct ggml_tensor * src1 = node->src[1];
    const size_t src0_bytes = src0 ? ggml_nbytes(src0) : 0;
    const size_t src1_bytes = src1 ? ggml_nbytes(src1) : 0;
    const size_t dst_bytes = ggml_nbytes(node);
    fprintf(stderr,
        "CPU_PERF|op=%s|name=%s|shape=[%" PRId64 ",%" PRId64 ",%" PRId64 ",%" PRId64 "]"
        "|type=%s|src0_type=%s|src0_shape=[%" PRId64 ",%" PRId64 ",%" PRId64 ",%" PRId64 "]"
        "|src1_type=%s|src1_shape=[%" PRId64 ",%" PRId64 ",%" PRId64 ",%" PRId64 "]"
        "|src0_bytes=%zu|src1_bytes=%zu|dst_bytes=%zu|total_bytes=%zu"
        "|threads=%d|graph_id=%d|node_n=%d|fused=%d|start_us=%" PRId64 "|end_us=%" PRId64 "|duration_us=%" PRId64 "\n",
        ggml_op_name(node->op), node->name ? node->name : "unnamed",
        node->ne[0], node->ne[1], node->ne[2], node->ne[3], ggml_type_name(node->type),
        ...
        src0_bytes, src1_bytes, dst_bytes, src0_bytes + src1_bytes + dst_bytes,
        n_threads, graph_id, node_n, fused, start_us, end_us, end_us - start_us);
}
...
static thread_ret_t ggml_graph_compute_thread(void * data) {
    ...
    const struct ggml_tensor * trace_node = NULL;
    int trace_node_n = -1;
    int trace_fused = 0;
    ...
    for (int node_n = 0; node_n < cgraph->n_nodes && atomic_load_explicit(&tp->abort, memory_order_relaxed) != node_n; node_n++) {
        ...
        if (state->ith == 0 && ggml_cpu_operator_trace) {
            trace_start_us = (int64_t) ggml_time_us();
            trace_node = node;
            trace_node_n = node_n;
            trace_fused = 0;
            trace_pending = true;
        ...
        if (node_n + 1 < cgraph->n_nodes) {
            ...
            if (state->ith == 0 && ggml_cpu_operator_trace && trace_pending) {
                const int64_t end_us = (int64_t) ggml_time_us();
                ggml_cpu_operator_trace_log(trace_node, params.nth,
                    (int) atomic_load_explicit(&tp->n_graph, memory_order_relaxed),
                    trace_node_n, trace_fused, trace_start_us, end_us);
                trace_pending = false;
            }
        }
    }
    ...
    // The final compute node has no following node barrier.  Emit its timing
    // after the existing terminal barrier so all worker threads are included.
    if (state->ith == 0 && ggml_cpu_operator_trace && trace_pending) {
        const int64_t end_us = (int64_t) ggml_time_us();
        ggml_cpu_operator_trace_log(trace_node, params.nth,
            (int) atomic_load_explicit(&tp->n_graph, memory_order_relaxed),
...
void ggml_cpu_init(void) {
    ...
    if (is_first_call) {
        ...
        {
            const char * env = getenv("GGML_CPU_OPERATOR_TRACE");
            ggml_cpu_operator_trace = (env != NULL && atoi(env) == 1);
        }
