...
public:
    explicit ggml_cuda_nvtx_scope(const ggml_tensor * tensor) {
        char label[512];
    ...
    ~ggml_cuda_nvtx_scope() { nvtxRangePop(); }
...
#if defined(GGML_CUDA_NVTX)
    ggml_cuda_nvtx_scope nvtx_scope(dst);
#endif
    switch (dst->op) {
        case GGML_OP_ARGMAX:
    ...
    while (!graph_evaluated_or_captured) {
        ...
        if (!use_cuda_graph || cuda_graph_update_required) {
            ...
            for (int i = 0; i < cgraph->n_nodes; i++) {
                ...
                // Fused paths are launched directly from ggml_cuda_try_fuse and
                // therefore bypass ggml_cuda_compute_forward (where the regular
                // operator NVTX scope is installed).  Keep an explicit scope
                // around the fusion attempt so CUPTI runtime correlation can
                // attribute asynchronous MMQ/MMVQ launches to the graph node
                // that owns them.  The scope is intentionally limited to the
                // launch call: it does not infer ownership from device timing.
#if defined(GGML_CUDA_NVTX)
                ggml_cuda_nvtx_scope fusion_nvtx_scope(node);
#endif
